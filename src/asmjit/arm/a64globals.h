...
struct Inst : public BaseInst {
  ...
  enum Attributes : uint32_t {
    ...
    kAttributeCondFlagShift = 26,
    //! Condition code flag shift
    kAttributeCondFlagMask = 1u << kAttributeCondFlagShift,

    //! Condition code value shift.
    kAttributeCondCodeShift = 27u,
    //! Condition code value mask.
    kAttributeCondCodeMask = 0xFu << kAttributeCondCodeShift,

    //! Mask of all attributes.
    kAttributesMask = kAttributeCondFlagMask | kAttributeCondCodeMask
  };
  ...
  static inline bool isDefinedId(uint32_t instId) noexcept { return (instId & ~kAttributesMask) < _kIdCount; }
};
