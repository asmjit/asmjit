...
public:
  ...
  //! Alias of \ref new_vec128_f32x1() that matches aarch64 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec_s(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x1, std::forward<Args>(args)...); }

  //! Alias of \ref new_vec128_f64x1() that matches aarch64 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec_d(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x1, std::forward<Args>(args)...); }
