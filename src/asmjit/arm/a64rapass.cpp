...
static uint32_t getControlType(uint32_t instId) {
  switch (instId & ~Inst::kAttributesMask) {
    case Inst::kIdB:
    case Inst::kIdBr:
      if ((instId & Inst::kAttributeCondFlagMask))
...
Error RACFGBuilder::onInst(InstNode* inst, uint32_t& controlType, RAInstBuilder& ib) noexcept {
  ...
  if (Inst::isDefinedId(instId)) {
    ...
    controlType = getControlType(instId);
  }
