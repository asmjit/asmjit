...
#include "../core.h"
  ...
  #include "../a64.h"
#else
  #pragma message("ASMJIT_NO_UJIT wasn't defined, however, no backends were found. AsmJit won't compile!")
#endif
...
#if defined(ASMJIT_UJIT_X86)
using BackendCompiler = x86::Compiler;
...
using BackendCompiler = a64::Compiler;
...
struct C {                                                                                  \
  ...
  ASMJIT_INLINE_CONSTEXPR explicit C(T x) noexcept : _v(x) {}                               \
  ASMJIT_INLINE_CONSTEXPR C(const C& other) noexcept = default;                             \
                                                                                            \
  ASMJIT_INLINE_CONSTEXPR T value() const noexcept { return _v; }                           \
                                                                                            \
  ASMJIT_INLINE_CONSTEXPR T* valuePtr() noexcept { return &_v; }                            \
  ASMJIT_INLINE_CONSTEXPR const T* valuePtr() const noexcept { return &_v; }                \
                                                                                            \
  ASMJIT_INLINE_CONSTEXPR C& operator=(T x) noexcept { _v = x; return *this; };             \
  ASMJIT_INLINE_CONSTEXPR C& operator=(const C& x) noexcept { _v = x._v; return *this; }    \
                                                                                            \
  ASMJIT_INLINE_CONSTEXPR C operator+(T x) const noexcept { return C(_v + x); }             \
  ASMJIT_INLINE_CONSTEXPR C operator-(T x) const noexcept { return C(_v - x); }             \
  ASMJIT_INLINE_CONSTEXPR C operator*(T x) const noexcept { return C(_v * x); }             \
  ASMJIT_INLINE_CONSTEXPR C operator/(T x) const noexcept { return C(_v / x); }             \
                                                                                            \
  ASMJIT_INLINE_CONSTEXPR C operator+(const C& x) const noexcept { return C(_v + x._v); }   \
  ASMJIT_INLINE_CONSTEXPR C operator-(const C& x) const noexcept { return C(_v - x._v); }   \
  ASMJIT_INLINE_CONSTEXPR C operator*(const C& x) const noexcept { return C(_v * x._v); }   \
  ASMJIT_INLINE_CONSTEXPR C operator/(const C& x) const noexcept { return C(_v / x._v); }   \
                                                                                            \
  ...
  ASMJIT_INLINE_CONSTEXPR C& operator/=(const C& x) noexcept { _v /= x._v; return *this; }  \
                                                                                            \
  ASMJIT_INLINE_CONSTEXPR bool operator==(T x) const noexcept { return _v == x; }           \
  ASMJIT_INLINE_CONSTEXPR bool operator!=(T x) const noexcept { return _v != x; }           \
  ASMJIT_INLINE_CONSTEXPR bool operator> (T x) const noexcept { return _v >  x; }           \
  ASMJIT_INLINE_CONSTEXPR bool operator>=(T x) const noexcept { return _v >= x; }           \
  ASMJIT_INLINE_CONSTEXPR bool operator< (T x) const noexcept { return _v <  x; }           \
  ASMJIT_INLINE_CONSTEXPR bool operator<=(T x) const noexcept { return _v <= x; }           \
                                                                                            \
  ASMJIT_INLINE_CONSTEXPR bool operator==(const C& x) const noexcept { return _v == x._v; } \
  ASMJIT_INLINE_CONSTEXPR bool operator!=(const C& x) const noexcept { return _v != x._v; } \
  ASMJIT_INLINE_CONSTEXPR bool operator> (const C& x) const noexcept { return _v >  x._v; } \
  ASMJIT_INLINE_CONSTEXPR bool operator>=(const C& x) const noexcept { return _v >= x._v; } \
  ASMJIT_INLINE_CONSTEXPR bool operator< (const C& x) const noexcept { return _v <  x._v; } \
...
ASMJIT_DEFINE_STRONG_TYPE(Alignment, uint32_t)
...
static ASMJIT_INLINE OperandSignature signatureOf(VecWidth vw) noexcept {
  RegType regType = RegType(uint32_t(RegType::kVec128) + uint32_t(vw));
  uint32_t regSize = 16u << uint32_t(vw);

  return OperandSignature::fromRegTypeAndGroup(regType, RegGroup::kVec) | OperandSignature::fromSize(regSize);
}

static ASMJIT_INLINE TypeId typeIdOf(VecWidth vw) noexcept {
#if defined(ASMJIT_UJIT_X86)
...
static ASMJIT_INLINE_NODEBUG VecWidth vecWidthOf(const Vec& reg) noexcept {
  return VecWidth(uint32_t(reg.regType()) - uint32_t(RegType::kVec128));
}

static ASMJIT_INLINE_NODEBUG VecWidth vecWidthOf(VecWidth vw, DataWidth dataWidth, uint32_t n) noexcept {
  return VecWidth(Support::min<uint32_t>((n << uint32_t(dataWidth)) >> 5, uint32_t(vw)));
}

static ASMJIT_INLINE Vec cloneVecAs(const Vec& src, VecWidth vw) noexcept {
  Vec result(src);
...
public:
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool empty() const noexcept { return _size == 0u; }

  //! Tests whether the vector has only one element, which makes it scalar.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool isScalar() const noexcept { return _size == 1u; }

  //! Tests whether the vector has more than 1 element, which means that calling `high()` and `odd()` won't return
  //! an empty vector.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool isVector() const noexcept { return _size > 1u; }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG size_t maxSize() const noexcept { return kMaxSize; }
...
public:
  ...
  ASMJIT_INLINE Vec& atUnrestricted(size_t index) noexcept {
    ASMJIT_ASSERT(index < kMaxSize);
    return static_cast<Vec&>(v[index]);
  }

  ASMJIT_INLINE const Vec& atUnrestricted(size_t index) const noexcept {
    ASMJIT_ASSERT(index < kMaxSize);
    return static_cast<const Vec&>(v[index]);
  }

  ASMJIT_INLINE void reassign(size_t index, const Vec& newVec) noexcept {
    ASMJIT_ASSERT(index < kMaxSize);
    v[index] = newVec;
  }
  ...
  ASMJIT_INLINE_NODEBUG void truncate(size_t newSize) noexcept {
    _size = Support::min(_size, newSize);
  }

  ASMJIT_INLINE VecArray cloneAs(OperandSignature signature) const noexcept {
    VecArray out(*this);
  ...
  ASMJIT_INLINE_NODEBUG VecWidth vecWidth() const noexcept { return VecWidthUtils::vecWidthOf(v[0].as<Vec>()); }

  ASMJIT_INLINE_NODEBUG void setVecWidth(VecWidth vw) noexcept {
    OperandSignature signature = VecWidthUtils::signatureOf(vw);
    for (size_t i = 0; i < size(); i++) {
      v[i].setSignature(signature);
    }
  }

  ASMJIT_INLINE_NODEBUG VecArray cloneAs(VecWidth vw) const noexcept { return cloneAs(VecWidthUtils::signatureOf(vw)); }
  ASMJIT_INLINE_NODEBUG VecArray cloneAs(const BaseReg& reg) const noexcept { return cloneAs(reg.signature()); }

  ASMJIT_INLINE_NODEBUG bool isVec128() const noexcept { return v[0].isVec128(); }
  ASMJIT_INLINE_NODEBUG bool isVec256() const noexcept { return v[0].isVec256(); }
  ASMJIT_INLINE_NODEBUG bool isVec512() const noexcept { return v[0].isVec512(); }

#if defined(ASMJIT_UJIT_X86)
  ASMJIT_INLINE_NODEBUG VecArray xmm() const noexcept { return cloneAs(OperandSignature{x86::Xmm::kSignature}); }
  ASMJIT_INLINE_NODEBUG VecArray ymm() const noexcept { return cloneAs(OperandSignature{x86::Ymm::kSignature}); }
  ASMJIT_INLINE_NODEBUG VecArray zmm() const noexcept { return cloneAs(OperandSignature{x86::Zmm::kSignature}); }
#endif
...
template<typename T>
static ASMJIT_INLINE void resetVarArray(T* array, size_t size) noexcept {
  for (size_t i = 0; i < size; i++)
    array[i].reset();
}

template<typename T>
static ASMJIT_INLINE void resetVarStruct(T* data, size_t size = sizeof(T)) noexcept {
  resetVarArray(reinterpret_cast<asmjit::BaseReg*>(data), size / sizeof(asmjit::BaseReg));
}

static ASMJIT_INLINE_NODEBUG const Operand_& firstOp(const Operand_& operand) noexcept { return operand; }
static ASMJIT_INLINE_NODEBUG const Operand_& firstOp(const OpArray& opArray) noexcept { return opArray[0]; }

static ASMJIT_INLINE_NODEBUG size_t countOp(const Operand_&) noexcept { return 1u; }
static ASMJIT_INLINE_NODEBUG size_t countOp(const OpArray& opArray) noexcept { return opArray.size(); }
...
static ASMJIT_INLINE_CONSTEXPR uint8_t perm2x128Imm(Perm2x128 hi, Perm2x128 lo) noexcept {
...
public:
  ...
  ASMJIT_INLINE ScopedInjector(BaseCompiler* cc, BaseNode** hook) noexcept
    : _cc(cc),
      _hook(hook),
      _prev(cc->setCursor(*hook)),
      _hookWasCursor(*hook == _prev) {}

  ASMJIT_INLINE ~ScopedInjector() noexcept {
    *_hook = _cc->cursor();
    if (!_hookWasCursor) {
      _cc->_setCursor(_prev);
    }
  }
