...
Vec UniCompiler::simdVecConst(const void* c, Bcst bcstWidth, VecWidth constWidth) noexcept {
  ...
  for (size_t i = 0; i < n; i++) {
    if (_vecConsts[i].ptr == c) {
      return Vec(OperandSignature{RegTraits<RegType::kVec128>::kSignature}, _vecConsts[i].vRegId);
    }
  }

  return Vec(OperandSignature{RegTraits<RegType::kVec128>::kSignature}, _newVecConst(c, true).id());
}
...
Vec UniCompiler::simdConst16B(const void* data16) noexcept {
  ...
  for (size_t i = 0; i < n; i++) {
    if (memcmp(_vecConstsEx[i].data, data16, 16) == 0) {
      return Vec(OperandSignature{RegTraits<RegType::kVec128>::kSignature}, _vecConstsEx[i].vRegId);
    }
  }
...
void UniCompiler::emit_rm(UniOpRM op, const Gp& dst, const Mem& src) noexcept {
  ...
  switch (op) {
    ...
    case UniOpRM::kLoadU64: {
      if (op == UniOpRM::kLoadI32 && dst.isGp64()) {
...
void UniCompiler::emit_2v(UniOpVV op, const Operand_& dst_, const Operand_& src_) noexcept {
  ...
  switch (op) {
    ...
    case UniOpVV::kSqrtF32S: {
      dst.setSignature(RegTraits<RegType::kVec32>::kSignature);
      Vec src = as_vec(this, src_, dst);
    ...
    case UniOpVV::kSqrtF64S: {
      dst.setSignature(RegTraits<RegType::kVec64>::kSignature);
      Vec src = as_vec(this, src_, dst);
...
void UniCompiler::emit_2vs(UniOpVR op, const Operand_& dst_, const Operand_& src_, uint32_t idx) noexcept {
  ...
  switch (op) {
    ...
    case UniOpVR::kInsertU64: {
      ...
      src.setSignature(op == UniOpVR::kInsertU64 ? RegTraits<RegType::kGp64>::kSignature : RegTraits<RegType::kGp32>::kSignature);
    ...
    case UniOpVR::kExtractU64: {
      ...
      dst.setSignature(op == UniOpVR::kExtractU64 ? RegTraits<RegType::kGp64>::kSignature : RegTraits<RegType::kGp32>::kSignature);
