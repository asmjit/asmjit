...
public:
  ...
  ASMJIT_NOINLINE void newRegArray(OpArray& dst, uint32_t n, const asmjit::Reg& ref, const char* name) noexcept {
    ASMJIT_ASSERT(n <= OpArray::kMaxSize);
    dst._size = n;
    for (uint32_t i = 0; i < n; i++) {
  ...
  ASMJIT_NOINLINE void newRegArray(OpArray& dst, uint32_t n, const asmjit::Reg& ref, const char* prefix, const char* name) noexcept {
    ASMJIT_ASSERT(n <= OpArray::kMaxSize);
    dst._size = n;
    for (uint32_t i = 0; i < n; i++) {
  ...
  #define DEFINE_OP_VM_A(name, op, default_alignment) \
    ASMJIT_INLINE void name(const Vec& dst, const Mem& src, Alignment alignment = Alignment{default_alignment}) noexcept { emit_vm(op, dst, src, alignment.v); } \
    ASMJIT_INLINE void name(const VecArray& dst, const Mem& src, Alignment alignment = Alignment{default_alignment}) noexcept { emit_vm(op, dst, src, alignment.v); }
  ...
  #define DEFINE_OP_MV_A(name, op, default_alignment) \
    ASMJIT_INLINE void name(const Mem& dst, const Vec& src, Alignment alignment = Alignment{default_alignment}) noexcept { emit_mv(op, dst, src, alignment.v); } \
    ASMJIT_INLINE void name(const Mem& dst, const VecArray& src, Alignment alignment = Alignment{default_alignment}) noexcept { emit_mv(op, dst, src, alignment.v); }
