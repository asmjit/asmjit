...
static constexpr UniOpVInfo opcode_info_2v[size_t(UniOpVV::kMaxValue) + 1] = {
  ...
  DEFINE_OP(kIdRoundss    , 2, kIntrin, kIdVroundss       , kIntrin     , 0, 0, kNone, 1, 0x08u, kF32S, k32, 4, kNA), // kRoundHalfUp32S.
  DEFINE_OP(kIdRoundsd    , 2, kIntrin, kIdVroundsd       , kIntrin     , 0, 0, kNone, 1, 0x08u, kF64S, k64, 8, kNA), // kRoundHalfUp64S.
  DEFINE_OP(kIdRoundps    , 2, kIntrin, kIdVroundps       , kIntrin     , 0, 0, kNone, 1, 0x08u, kF32V, k32, 4, kNA), // kRoundHalfUp32.
  DEFINE_OP(kIdRoundpd    , 2, kIntrin, kIdVroundpd       , kIntrin     , 0, 0, kNone, 1, 0x08u, kF64V, k64, 8, kNA), // kRoundHalfUp64.
...
static ASMJIT_NOINLINE void sse_round(UniCompiler& uc, const Vec& dst, const Operand& src, FloatMode fm, x86::RoundImm round_mode) noexcept {
  ...
  if (fm == FloatMode::kF32S || fm == FloatMode::kF32V)
    maxn = uc.simd_const(&uc.ct().f32_round_magic, Bcst::k32, dst);
  else
    maxn = uc.simd_const(&uc.ct().f64_round_magic, Bcst::k64, dst);
  ...
  Operand one;
  if (fm == FloatMode::kF32S || fm == FloatMode::kF32V)
    one = uc.simd_const(&uc.ct().f32_1, Bcst::k32, dst);
  else
    one = uc.simd_const(&uc.ct().f64_1, Bcst::k64, dst);
  ...
  if (has_avx()) {
    ...
    switch (op) {
      ...
      case UniOpVV::kRoundHalfUpF64: {
        ...
        Operand half;
        if (is_f32_op(fm))
          half = simd_const(&ct().f32_0_5, Bcst::k32, dst);
        else
          half = simd_const(&ct().f64_0_5, Bcst::k64, dst);
    ...
    switch (op) {
      ...
      case UniOpVV::kRoundHalfUpF64: {
        ...
        emit_2v(translate_op(op, UniOpVV::kRoundHalfUpF32S, UniOpVV::kFloorF32S), dst, dst);
        return;
      }
