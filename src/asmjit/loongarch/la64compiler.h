...
#ifndef ASMJIT_LA_A64COMPILER_H_INCLUDED
#define ASMJIT_LA_A64COMPILER_H_INCLUDED
...
//! Loongarch compiler implementation.
...
public:
  ...
  ASMJIT_INLINE_NODEBUG Error invoke_(InvokeNode** out, const Operand_& target, const FuncSignature& signature) {
    return addInvokeNode(out, Inst::kIdBl, target, signature);
  }
  ...
  using EmitterExplicitT<Compiler>::bl;

  //! Adds a jump to the given `target` with the provided jump `annotation`.
  ASMJIT_INLINE_NODEBUG Error br(const BaseReg& target, JumpAnnotation* annotation) { return emitAnnotatedJump(Inst::kIdBl, target, annotation); }
...
#endif // ASMJIT_LA_A64COMPILER_H_INCLUDED
