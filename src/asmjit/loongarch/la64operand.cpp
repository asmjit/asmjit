...
UNIT(a64_operand) {
  ...
  EXPECT_EQ(w0.type(), RegType::kLA_GpW);
  EXPECT_EQ(x0.type(), RegType::kLA_GpX);
  EXPECT_EQ(w0.group(), RegGroup::kGp);
  EXPECT_EQ(x0.group(), RegGroup::kGp);

  INFO("Checking Vec register properties");
  EXPECT_EQ(v0.type(), RegType::kLA_VecV);
  EXPECT_EQ(d0.type(), RegType::kLA_VecD);
  EXPECT_EQ(s0.type(), RegType::kLA_VecS);
  EXPECT_EQ(h0.type(), RegType::kLA_VecH);
  EXPECT_EQ(b0.type(), RegType::kLA_VecB);
  ...
  Vec vd_1 = v15.d(1);
  EXPECT_EQ(vd_1.type(), RegType::kLA_VecV);
  ...
  Vec vs_3 = v15.s(3);
  EXPECT_EQ(vs_3.type(), RegType::kLA_VecV);
  ...
  Vec vb_4 = v15.b4(3);
  EXPECT_EQ(vb_4.type(), RegType::kLA_VecV);
