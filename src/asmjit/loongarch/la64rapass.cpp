...
// a64::LARAPass - Helpers
// ========================

// TODO: [LA] These should be shared with all backends.
ASMJIT_MAYBE_UNUSED
...
static const RegMask raConsecutiveLeadCountToRegMaskFilter[5] = {
  ...
  0x7FFFFFF0u, // [2] 2 consecutive registers.
  0x3FFFFFF0u, // [3] 3 consecutive registers.
  0x1FFFFFF0u  // [4] 4 consecutive registers.
};
...
public:
  ...
  inline RACFGBuilder(LARAPass* pass) noexcept
        ...
        if (op.isReg()) {
          ...
          if (mem.isRegHome()) {
            ...
            if (vIndex < Operand::kVirtIdCount) {
              ...
              // Base registers have never fixed id on LA.
          ...
          if (mem.hasIndexReg()) {
            ...
            if (vIndex < Operand::kVirtIdCount) {
              ...
              // Index registers have never fixed id on LA.
              const uint32_t useId = BaseReg::kIdBad;
              const uint32_t outId = BaseReg::kIdBad;
      ...
      if (op.isReg()) {
        ...
        if (arg.isReg()) {
          ...
          if (regGroup != argGroup) {
            // TODO: [LA] Conversion is not supported.
...
Error RACFGBuilder::moveImmToRegArg(InvokeNode* invokeNode, const FuncValue& arg, const Imm& imm_, BaseReg* out) noexcept {
  ...
  return cc()->addi_d(out->as<Gp>(), zero, imm);  //FIXME imm range
}
...
Error RACFGBuilder::moveRegToStackArg(InvokeNode* invokeNode, const FuncValue& arg, const BaseReg& reg) noexcept {
  ...
  if (reg.isGp())
    return cc()->st_d(reg.as<Gp>(), stackPtr);

  if (reg.isVec())
    return cc()->vst(reg.as<Vec>(), stackPtr);
...
// a64::LARAPass - Construction & Destruction
// ===========================================

LARAPass::LARAPass() noexcept
  : BaseRAPass() { _iEmitHelper = &_emitHelper; }
LARAPass::~LARAPass() noexcept {}

// a64::LARAPass - OnInit / OnDone
// ================================

void LARAPass::onInit() noexcept {
  Arch arch = cc()->arch();
  ...
  _availableRegs[RegGroup::kGp] = 0x1ff000;  //t0~t8 FIXME add more Reg to use
...
void LARAPass::onDone() noexcept {}

// a64::LARAPass - BuildCFG
// =========================

Error LARAPass::buildCFG() noexcept {
  return RACFGBuilder(this).run();
}

// a64::LARAPass - Rewrite
// ========================

ASMJIT_FAVOR_SPEED Error LARAPass::_rewrite(BaseNode* first, BaseNode* stop) noexcept {
  ...
  while (node != stop) {
    ...
    if (node->isInst()) {
      ...
      if (0 && inst->opCount() == 2 && inst->op(1).isMem()) {  //FIXME
        BaseMem mem = inst->op(1).as<BaseMem>();
        int64_t offset = mem.offset();

        if (!mem.hasBaseOrIndex()) {
          inst->setId(Inst::kIdMove);
          ...
          InstId arithInstId = offset < 0 ? Inst::kIdSub_d : Inst::kIdAdd_d;
          ...
          if (absOffset > 0xFFFu && (absOffset & ~uint64_t(0xFFF000u)) != 0) {
            if (absOffset <= 0xFFFFFFu) {
              ...
              cc()->_setCursor(inst->prev());
              ASMJIT_PROPAGATE(cc()->emit(Inst::kIdMove, inst->op(0), Imm(absOffset)));
...
// a64::LARAPass - Prolog & Epilog
// ================================

Error LARAPass::updateStackFrame() noexcept {
...
Error LARAPass::emitMove(uint32_t workId, uint32_t dstPhysId, uint32_t srcPhysId) noexcept {
...
Error LARAPass::emitSwap(uint32_t aWorkId, uint32_t aPhysId, uint32_t bWorkId, uint32_t bPhysId) noexcept {
  DebugUtils::unused(aWorkId, aPhysId, bWorkId, bPhysId);
  return DebugUtils::errored(kErrorInvalidState);
}

Error LARAPass::emitLoad(uint32_t workId, uint32_t dstPhysId) noexcept {
  RAWorkReg* wReg = workRegById(workId);
...
Error LARAPass::emitSave(uint32_t workId, uint32_t srcPhysId) noexcept {
  RAWorkReg* wReg = workRegById(workId);
...
Error LARAPass::emitJump(const Label& label) noexcept {
  return cc()->b(label);
}

Error LARAPass::emitPreCall(InvokeNode* invokeNode) noexcept {
  DebugUtils::unused(invokeNode);
  return kErrorOk;
}
