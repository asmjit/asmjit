...
public:
  ...
  static inline constexpr size_t kMinAlignment = Globals::kZoneAlignment;
  ...
  static ASMJIT_INLINE_CONSTEXPR size_t alignedSizeOf() noexcept {
    return Support::alignUp(sizeof(T), kMinAlignment);
  }
  ...
  //! Internal alloc function with `alignment` used by other inline functions.
  [[nodiscard]]
