...
struct Operand_ {
  ...
  [[nodiscard]]
  ASMJIT_INLINE_CONSTEXPR bool isMaskReg() const noexcept { return isReg(RegGroup::kMask); }

  //! Tests whether the register is a mask register of any size having the given id `regId` (X86/X86_64/AArch64).
  [[nodiscard]]
  ASMJIT_INLINE_CONSTEXPR bool isMaskReg(uint32_t regId) const noexcept { return isReg(RegGroup::kMask, regId); }
...
ASMJIT_DEFINE_REG_TRAITS(RegType::kVec1024       , RegGroup::kVec         , 128, TypeId::kInt32x16); // | x | x |   |   |
...
ASMJIT_DEFINE_REG_TRAITS(RegType::kX86_SReg      , RegGroup::kX86_SReg    , 2  , TypeId::kVoid    ); // | x | x |   |   |
ASMJIT_DEFINE_REG_TRAITS(RegType::kX86_CReg      , RegGroup::kX86_CReg    , 0  , TypeId::kVoid    ); // | x | x |   |   |
ASMJIT_DEFINE_REG_TRAITS(RegType::kX86_DReg      , RegGroup::kX86_DReg    , 0  , TypeId::kVoid    ); // | x | x |   |   |
...
public:
  ...
  //! returns false.
  [[nodiscard]]
  ASMJIT_INLINE_CONSTEXPR bool isSame(const BaseReg& other) const noexcept { return (_signature == other._signature) & (_baseId == other._baseId); }
...
public:
  ...
  ASMJIT_INLINE_CONSTEXPR void setBase(const BaseReg& base) noexcept { return _setBase(base.regType(), base.id()); }

  //! Sets the index register to type and id of the given `index` operand.
  ASMJIT_INLINE_CONSTEXPR void setIndex(const BaseReg& index) noexcept { return _setIndex(index.regType(), index.id()); }
