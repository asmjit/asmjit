...
static ASMJIT_NOINLINE void BaseEmitter_updateForcedOptions(BaseEmitter* self) noexcept {
  ...
  if (self->emitterType() == EmitterType::kAssembler) {
    ...
    hasValidationOptions = self->hasValidationOption(ValidationOptions::kValidateAssembler);
  }
  else {
    // Builder/Compiler: Always emit comments, we cannot assume they won't be used.
    emitComments = self->_code != nullptr;
    hasValidationOptions = self->hasValidationOption(ValidationOptions::kValidateIntermediate);
  }
...
// BaseEmitter - Validation Options
// ================================

void BaseEmitter::addValidationOptions(ValidationOptions options) noexcept {
  _validationOptions |= options;
  BaseEmitter_updateForcedOptions(this);
}

void BaseEmitter::clearValidationOptions(ValidationOptions options) noexcept {
  _validationOptions &= ~options;
  BaseEmitter_updateForcedOptions(this);
}
