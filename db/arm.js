...
class Instruction extends base.Instruction {
  ...
  _assignMetadata(metadata) {
    for (let k in metadata) {
      if (hasOwn.call(IGNORE_FIELDS, k))
        continue;

      const v = metadata[k];
      switch (k) {
        case "ext":
          for (let ext of v.split(" "))
            this.extensions[ext.trim()] = true;
          break;

        case "it":
          for (let it of v.split("|"))
            this.it[it.trim()] = true;
          break;

        case "apsr":
        case "fpcsr":
          this._assignAttributeKeyValue(k, v);
          break;

        case "imm":
          this.imm = exp.parse(v);
          break;

        case "immCond":
          console.log(`WARNING: ${this.name}: immCond '${v}' ignored`);
          break;

        case "calc":
          for (let calcKey in v)
            this.calc[calcKey] = exp.parse(v[calcKey]);
          break;
        ...
        default:
          FAIL(`Instruction '${this.name}' contains unknown metadata key '${k}'`);
  ...
  _assignOpcode(s) {
    ...
    // this.opcodePattern = Object.keys(patternMap).sort().join("|");
    this.opcodePattern = pattern.join("|").replace(/_\|_/g, "_").replace(/_\|_/g, "_");
