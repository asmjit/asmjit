...
static void print_type_sizes(void) noexcept {
  ...
  printf("Size of arena classes:\n");
    DUMP_TYPE(axl::Arena);
    DUMP_TYPE(axl::ArenaHashNode);
    DUMP_TYPE(axl::ArenaHash<axl::ArenaHashNode>);
    DUMP_TYPE(axl::ArenaList<int>);
    DUMP_TYPE(axl::ArenaVector<int>);
    DUMP_TYPE(axl::ArenaString<16>);
  printf("\n");
...
int main() {
  printf("AsmJit Test Struct Sizes v%u.%u.%u [Arch=%s] [Mode=%s]\n\n",
