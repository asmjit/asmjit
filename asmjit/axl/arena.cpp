...
static constexpr size_t kArenaAlignmentOverhead =
  (Arena::kAlignment <= axl::kAllocAlignment) ? size_t(0) : Arena::kAlignment - axl::kAllocAlignment;
...
static ASMJIT_INLINE void* Arena_malloc(size_t size) noexcept { return ::malloc(size); }
static ASMJIT_INLINE void Arena_free(void* ptr) noexcept { ::free(ptr); }

static ASMJIT_INLINE void Arena_assign_block(Arena& arena, Arena::ManagedBlock* block) noexcept {
  arena._ptr = axl::align_up(block->data(), Arena::kAlignment);
...
void Arena::_init(size_t min_block_size, Span<uint8_t> static_arena_memory) noexcept {
  ASMJIT_ASSERT(min_block_size >= kMinBlockSize);
  ASMJIT_ASSERT(min_block_size <= kMaxBlockSize);

  ManagedBlock* block = Arena_get_zero_block();
  size_t block_size_shift = axl::bit_size_of<size_t> - axl::clz(min_block_size);
  ...
  if (static_arena_memory.size()) {
    block = reinterpret_cast<ManagedBlock*>(static_arena_memory.data_mut());
    block->next = nullptr;
...
void Arena::_reset(bool release_memory) noexcept {
...
void* Arena::_alloc_oneshot(size_t size) noexcept {
  ...
  ASMJIT_ASSERT(axl::is_aligned(size, Arena::kAlignment));
  ...
  constexpr size_t kBlockSizeOverhead = sizeof(ManagedBlock) + axl::kAllocOverhead + kArenaAlignmentOverhead;
  ...
  while (next) {
    uint8_t* ptr = axl::align_up(next->data(), Arena::kAlignment);
  ...
  ASMJIT_ASSERT(axl::is_aligned(new_block, axl::kAllocAlignment));
  ...
  uint8_t* ptr = axl::align_up(new_block->data(), Arena::kAlignment);
  ...
  _current_block_size_shift = uint8_t(axl::min<uint32_t>(uint32_t(block_size_shift) + 1u, _max_block_size_shift));
  _unused_byte_count += unused_byte_count;
...
void* Arena::dup(const void* data, size_t size, bool null_terminate) noexcept {
  ...
  size_t alloc_size = axl::align_up(size + size_t(null_terminate), Arena::kAlignment);
  ...
  axl::storeu<uint64_t>(m + alloc_size - sizeof(uint64_t), 0u);
...
void* Arena::_alloc_reusable(size_t size, Out<size_t> allocated_size) noexcept {
  ...
  if (_get_reusable_slot_index(size, Out(slot), allocated_size)) {
    ...
    size_t dynamic_block_overhead = axl::align_up(sizeof(DynamicBlock) + sizeof(DynamicBlock*) + kArenaAlignmentOverhead, kAlignment);
    ...
    // If this doesn't hold the whole code is broken as we would use more space than allocated due to call to align_up().
    ASMJIT_ASSERT(axl::is_aligned(p, axl::kAllocAlignment));
    ...
    p = axl::align_up(static_cast<uint8_t*>(p) + sizeof(DynamicBlock) + sizeof(DynamicBlock*), kAlignment);
...
ArenaUsage Arena::usage() const noexcept {
  ...
  ArenaUsage out {};
  out._block_count = block_count;
  out._used_size = used_size;
  out._reserved_size = reserved_size;
  out._overhead_size = _unused_byte_count;
  return out;
}
