...
#ifndef ASMJIT_AXL_ARENA_BIT_SET_P_H_INCLUDED
#define ASMJIT_AXL_ARENA_BIT_SET_P_H_INCLUDED
...
template<typename T>
[[nodiscard]]
ASMJIT_INLINE_NODEBUG size_t size_in_bits(const Span<T>& span) noexcept { return span.size() * bit_size_of<T>; }

template<typename T>
[[nodiscard]]
ASMJIT_INLINE_NODEBUG size_t size_in_words(size_t num_bits) noexcept { return (num_bits + bit_size_of<T> - 1u) / bit_size_of<T>; }

template<typename T, typename Index>
[[nodiscard]]
ASMJIT_INLINE bool bit_at(const Span<T>& span, const Index& index) noexcept {
  size_t i = as_basic_uint(index);
  size_t word_index = i / bit_size_of<T>;
  size_t bit_index = i % bit_size_of<T>;
...
ASMJIT_INLINE void set_bit(const Span<T>& span, const Index& index, bool value) noexcept {
  ...
  span[word_index] = T((span[word_index] & and_mask) | bit_mask);
}
...
ASMJIT_INLINE void clear_bit(const Span<T>& span, const Index& index) noexcept {
  ...
  T and_mask = T(~(T(1u) << bit_index));
  span[word_index] = T(span[word_index] & and_mask);
}
...
ASMJIT_INLINE void or_bit(const Span<T>& span, const Index& index, bool value) noexcept {
  ...
  T bit_mask = T(T(value) << bit_index);
  span[word_index] = T(span[word_index] | bit_mask);
}
...
ASMJIT_INLINE void or_(Span<T> dst, Args&&... args) noexcept {
  return combine_spans<Or>(dst, forward<Args>(args)...);
}
...
public:
  ...
  static ASMJIT_INLINE_NODEBUG size_t _words_per_bits(size_t num_bits) noexcept {
    return ((num_bits + bit_size_of<BitWord> - 1u) / bit_size_of<BitWord>);
  }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<const BitWord> span() const noexcept { return Span<BitWord>(_data, size_in_bit_words()); }

  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<BitWord> span_mut() noexcept { return Span<BitWord>(_data, size_in_bit_words()); }
  ...
  ASMJIT_INLINE bool bit_at(const Index& index) const noexcept {
    ASMJIT_ASSERT(as_basic_uint(index) < _size);
    return bit_vector_get_bit(_data, as_basic_uint(index));
  }

  template<typename Index>
  ASMJIT_INLINE void set_bit(const Index& index, bool value) noexcept {
    ASMJIT_ASSERT(as_basic_uint(index) < _size);
    bit_vector_set_bit(_data, as_basic_uint(index), value);
  }

  template<typename Index>
  ASMJIT_INLINE void add_bit(const Index& index, bool value) noexcept {
    ASMJIT_ASSERT(as_basic_uint(index) < _size);
    bit_vector_or_bit(_data, as_basic_uint(index), value);
  }

  template<typename Index>
  ASMJIT_INLINE void clear_bit(const Index& index) noexcept {
    ASMJIT_ASSERT(as_basic_uint(index) < _size);
    bit_vector_set_bit(_data, as_basic_uint(index), false);
  }

  template<typename Index>
  ASMJIT_INLINE void xor_bit(const Index& index, bool value) noexcept {
    ASMJIT_ASSERT(as_basic_uint(index) < _size);
    bit_vector_xor_bit(_data, as_basic_uint(index), value);
  }
  ...
  ASMJIT_INLINE Error append(Arena& arena, bool value) noexcept {
    ...
    uint32_t idx = index / bit_size_of<BitWord>;
    uint32_t bit = index % bit_size_of<BitWord>;
  ...
  ASMJIT_INLINE void clear_bits(size_t start, size_t count) noexcept {
    ...
    bit_vector_clear(_data, start, count);
  }
  ...
  ASMJIT_INLINE void fill_bits(size_t start, size_t count) noexcept {
    ...
    bit_vector_fill(_data, start, count);
  }
  ...
  ASMJIT_INLINE void and_(const ArenaBitSet& other) noexcept {
    ...
    size_t common_bit_word_count = min(this_bit_word_count, other_bit_word_count);
  ...
  ASMJIT_INLINE void and_not(const ArenaBitSet& other) noexcept {
    ...
    size_t common_bit_word_count = _words_per_bits(min(_size, other._size));
  ...
  ASMJIT_INLINE void _clear_unused_bits() noexcept {
    uint32_t idx = _size / bit_size_of<BitWord>;
    uint32_t bit = _size % bit_size_of<BitWord>;
  ...
  class ForEachBitSet : public BitVectorIterator<BitWord> {
  public:
    inline explicit ForEachBitSet(const ArenaBitSet& bit_vector) noexcept
      : BitVectorIterator<BitWord>(bit_vector.span()) {}
  };
...
#endif // ASMJIT_AXL_ARENA_BIT_SET_P_H_INCLUDED
