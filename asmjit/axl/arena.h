...
#ifndef ASMJIT_AXL_ARENA_H_INCLUDED
#define ASMJIT_AXL_ARENA_H_INCLUDED
...
struct ArenaUsage {
  ...
  ASMJIT_INLINE void aggregate(const ArenaUsage& other) noexcept {
  ...
  ASMJIT_INLINE ArenaUsage& operator+=(const ArenaUsage& other) noexcept {
    aggregate(other);
    return *this;
  }
...
public:
  ...
  static inline constexpr size_t kMinBlockSize = 1024;
  //! Maximum managed block size.
  static inline constexpr size_t kMaxBlockSize = size_t(1) << (sizeof(size_t) * 8 - 2);
  ...
  static ASMJIT_INLINE bool _get_reusable_slot_index(size_t size, Out<size_t> slot) noexcept {
    slot = axl::bit_size_of<size_t> - 4u - axl::clz((size - 1u) | 0xF);
  ...
  template<typename T>
  static ASMJIT_INLINE_CONSTEXPR size_t aligned_size_of() noexcept { return axl::align_up(sizeof(T), kAlignment); }

  static ASMJIT_INLINE_CONSTEXPR size_t aligned_size(size_t size) noexcept { return axl::align_up(size, kAlignment); }
  ...
  //! Current block size shift - reverted to _min_block_size_shift every time the Arena is `_reset(true)`.
  ...
  //! True when the Arena has a static block (initial inline block passed via \ref Span<uint8_t>).
  ...
  //!
  //! This will destroy the `Arena` instance and release all blocks of memory allocated by it. All pointers returned
  //! by `alloc_xxx()` functions will get invalidated.
  ASMJIT_INLINE_NODEBUG ~Arena() noexcept { _reset(true); }

  ASMJIT_API void _init(size_t min_block_size, Span<uint8_t> static_arena_memory) noexcept;

  //! Internal reset functionality that can be used with a `bool` parameter that describes whether to release allocated
  //! blocks or not.
  //!
  //! \remarks This function invalidates all blocks of memory allocated by it.
  ASMJIT_API void _reset(bool release_memory) noexcept;

  //! Resets the arena, but reuses the allocated blocks unless they exceed a maximum block size.
  //!
  //! \remarks This function invalidates all blocks of memory allocated by it.
  ASMJIT_INLINE void clear() noexcept { return _reset(false); }

  //! Resets the `Arena` by releasing all memory it allocated.
  //!
  //! \remarks This function invalidates all blocks of memory allocated by it.
  ASMJIT_INLINE void release() noexcept { return _reset(true); }
  ...
  //! Tests whether this `Arena` has an inline/static block passed via \ref Span<uint8_t> during `Arena` construction.
  [[nodiscard]]
  ...
  ASMJIT_INLINE T* alloc_oneshot(size_t size) noexcept {
    ASMJIT_ASSERT(axl::is_aligned(size, kAlignment));
    ...
    if (__builtin_constant_p(size) && size < 4096u) {
  ...
  ASMJIT_INLINE T* new_oneshot() noexcept {
    ...
    return new(axl::PlacementNew{p}) T();
  }
  ...
  ASMJIT_INLINE T* new_oneshot(Args&&... args) noexcept {
    ...
    return new(axl::PlacementNew{p}) T(axl::forward<Args>(args)...);
  }
  ...
  //! Calculates and returns details about memory usage of this \ref Arena.
  //!
  //! \note This function fills all members, but `_pooled_size` member (see \ref ArenaUsage::pooled_size()
  ...
  ASMJIT_API ArenaUsage usage() const noexcept;
...
#endif // ASMJIT_AXL_ARENA_H_INCLUDED
