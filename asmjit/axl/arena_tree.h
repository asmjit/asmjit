...
#ifndef ASMJIT_AXL_ARENA_TREE_H_INCLUDED
#define ASMJIT_AXL_ARENA_TREE_H_INCLUDED
...
public:
  ...
  ASMJIT_INLINE_NODEBUG void swap(ArenaTree& other) noexcept {
    axl::swap(_root, other._root);
  }

  template<typename LessFn = Less>
  void insert(NodeT* ASMJIT_NONNULL(node), LessFn&& less_than_fn = LessFn{}) noexcept {
    ...
    for (;;) {
      ...
      last = dir;
      dir = size_t(less_than_fn(*static_cast<NodeT*>(q), *static_cast<NodeT*>(node)));
  ...
  template<typename LessFn = Less>
  void remove(ArenaTreeNode* ASMJIT_NONNULL(node), LessFn&& less_than_fn = LessFn{}) noexcept {
    ArenaTreeNode head;           // False root node,
    head._set_right(_root);       // having root on the right.
    ...
    if (f != q) {
      ...
      dir = (n == &head) ? size_t(1) : size_t(less_than_fn(*static_cast<NodeT*>(n), *static_cast<NodeT*>(node)));
      ...
      for (;;) {
        ...
        dir = size_t(less_than_fn(*static_cast<NodeT*>(n), *static_cast<NodeT*>(node)));
      }
    }
  ...
  template<typename KeyT, typename LessFn = Less>
  [[nodiscard]]
  inline NodeT* get(const KeyT& key, LessFn&& less_than_fn = LessFn{}) const noexcept {
    ArenaTreeNode* node = _root;
    while (node) {
      bool lt = less_than_fn(*static_cast<const NodeT*>(node), key);
      bool gt = less_than_fn(key, *static_cast<const NodeT*>(node));

      if (bool_and(!lt, !gt)) {
        break;
      }

      // Go left or right depending on where the key is.
      node = node->_get_child(size_t(lt));
    }
...
#endif // ASMJIT_AXL_ARENA_TREE_H_INCLUDED
