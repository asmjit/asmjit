...
#ifndef ASMJIT_AXL_ARENA_VECTOR_H_INCLUDED
#define ASMJIT_AXL_ARENA_VECTOR_H_INCLUDED
...
protected:
  ...
  ASMJIT_INLINE_NODEBUG void _swap(ArenaVectorBase& other) noexcept {
    ...
    axl::swap(_capacity, other._capacity);
  }
...
public:
  ...
  ASMJIT_INLINE_NODEBUG void truncate(size_t n) noexcept {
    _size = uint32_t(min<size_t>(_size, n));
  }
...
public:
  ...
  static inline constexpr bool ItemSizeIsPowerOf2 = is_power_of_2(sizeof(T));
  static inline constexpr ItemSize<ItemSizeIsPowerOf2> item_size_ { ItemSizeIsPowerOf2 ? ctz(sizeof(T)) : uint32_t(sizeof(T)) };
  ...
  using const_type = const_type_t<T>;
  ...
  ASMJIT_INLINE_NODEBUG ArenaVector(ArenaVector&& other) noexcept : ArenaVectorBase(move(other)) {}
  ...
  //! Returns a non-owning span of this vector (const).
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<const_type> span() const noexcept { return Span<const_type>(static_cast<const_type*>(_data), _size); }

  //! Returns a non-owning span of this vector (mutable).
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<T> span_mut() const noexcept { return Span<T>(static_cast<T*>(_data), _size); }
  ...
  ASMJIT_INLINE_NODEBUG SpanReverseView<T> reverse_view() const noexcept { return {static_cast<T*>(_data), _size}; }
  ...
  //!
  //! Can only be used together with `reserve_additional()`. If `reserve_additional(N)` succeeds then N elements
  ...
  ASMJIT_INLINE bool contains(Value&& value) const noexcept { return span().contains(forward<Value>(value)); }

  //! Returns the first index of the given `value` or `SIZE_MAX` if it wasn't found.
  template<typename Value>
  ASMJIT_INLINE size_t index_of(Value&& value) const noexcept { return span().index_of(forward<Value>(value)); }

  //! Returns the last index of the given `value` or `SIZE_MAX` if it wasn't found.
  template<typename Value>
  ASMJIT_INLINE size_t last_index_of(Value&& value) const noexcept { return span().index_of(forward<Value>(value)); }
  ...
  ASMJIT_INLINE Error _reserve_fit(Arena& arena, size_t n) noexcept { return ArenaVectorBase::_reserve_fit(arena, n, item_size_); }
  ASMJIT_INLINE Error _reserve_grow(Arena& arena, size_t n) noexcept { return ArenaVectorBase::_reserve_grow(arena, n, item_size_); }
  ASMJIT_INLINE Error _resize_fit(Arena& arena, size_t n) noexcept { return ArenaVectorBase::_resize_fit(arena, n, item_size_); }
  ASMJIT_INLINE Error _resize_grow(Arena& arena, size_t n) noexcept { return ArenaVectorBase::_resize_grow(arena, n, item_size_); }
  ASMJIT_INLINE Error _reserve_additional(Arena& arena, size_t n) noexcept { return ArenaVectorBase::_reserve_additional(arena, n, item_size_); }
  ...
  ASMJIT_INLINE void release(Arena& arena) noexcept { _release(arena, sizeof(T)); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE Error resize_fit(Arena& arena, size_t n) noexcept { return _resize_fit(arena, n); }
  ...
  //! If `n` is less than the current size then the vector will be truncated to exactly `n` elements.
  [[nodiscard]]
  ASMJIT_INLINE Error resize_grow(Arena& arena, size_t n) noexcept { return _resize_grow(arena, n); }
...
#endif // ASMJIT_AXL_ARENA_VECTOR_H_INCLUDED
