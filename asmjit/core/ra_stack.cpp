...
RAStackSlot* RAStackAllocator::new_slot(uint32_t base_reg_id, uint32_t size, uint32_t alignment, uint32_t flags) noexcept {
  ...
  slot->_alignment = uint8_t(axl::max<uint32_t>(alignment, 1));
  ...
  _alignment = axl::max<uint32_t>(_alignment, alignment);
...
Error RAStackAllocator::calculate_stack_frame() noexcept {
  ...
  for (RAStackSlot* slot : _slots) {
    ...
    uint32_t power = axl::min<uint32_t>(axl::ctz(alignment), 6);
    uint64_t weight;
  ...
  axl::sort(_slots.span(), [](const RAStackSlot* a, const RAStackSlot* b) noexcept -> bool {
  ...
  // alignment to `gaps`.
  uint32_t offset = 0;
  axl::ArenaVector<RAStackGap> gaps[kSizeCount - 1];
  ...
  for (RAStackSlot* slot : _slots) {
    ...
    uint32_t aligned_offset = axl::align_up(offset, slot_alignment);
...

    {
      ...
      if (slot_size < (1u << uint32_t(ASMJIT_ARRAY_SIZE(gaps)))) {
        ...
        uint32_t index = axl::ctz(slot_size);
        do {
          if (!gaps[index].is_empty()) {
            RAStackGap gap = gaps[index].pop();

            ASMJIT_ASSERT(axl::is_aligned(gap.offset, slot_alignment));
    ...
    if (gap_size) {
      ...
      while (gap_offset < gap_end) {
        uint32_t index = axl::ctz(gap_offset);
    ...
    if (!found_gap) {
      ASMJIT_ASSERT(axl::is_aligned(offset, slot_alignment));
      slot->set_offset(int32_t(offset));
      offset += slot->size();
    }
  }

  _stack_size = axl::align_up(offset, _alignment);
  return Error::kOk;
}
