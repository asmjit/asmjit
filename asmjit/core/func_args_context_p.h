...
#ifndef ASMJIT_CORE_FUNC_ARGS_CONTEXT_P_H_INCLUDED
#define ASMJIT_CORE_FUNC_ARGS_CONTEXT_P_H_INCLUDED
...
#include <asmjit/core/ra_constraints_p.h>
...
static inline OperandSignature get_suitable_reg_for_mem_to_mem_move(Arch arch, TypeId dst_type_id, TypeId src_type_id) noexcept {
  ...
  uint32_t max_size = axl::max<uint32_t>(dst_size, src_size);
...
public:
  ...
  struct WorkData {
    ...
    inline bool is_assigned(uint32_t reg_id) const noexcept {
      ...
      return axl::bit_test(_assigned_regs, reg_id);
    }
    ...
    inline void assign(uint32_t var_id, uint32_t reg_id) noexcept {
      ...
      _assigned_regs ^= axl::bit_mask<RegMask>(reg_id);
    }
    ...
    inline void reassign(uint32_t var_id, uint32_t new_id, uint32_t old_id) noexcept {
      ...
      _assigned_regs ^= axl::bit_mask<RegMask>(new_id) ^ axl::bit_mask<RegMask>(old_id);
    }
  ...
  axl::InplaceArray<WorkData, Globals::kNumVirtGroups> _work_data;
...
#endif // ASMJIT_CORE_FUNC_ARGS_CONTEXT_P_H_INCLUDED
