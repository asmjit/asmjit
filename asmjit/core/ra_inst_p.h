...
#ifndef ASMJIT_CORE_RA_INST_P_H_INCLUDED
#define ASMJIT_CORE_RA_INST_P_H_INCLUDED
...
public:
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_inst_rw_flag(InstRWFlags flag) const noexcept { return axl::test(_inst_rw_flags, flag); }
  ...
  static ASMJIT_INLINE_NODEBUG size_t size_of(uint32_t tied_reg_count) noexcept {
    return axl::Arena::aligned_size_of<RAInst>() - sizeof(RATiedReg) + tied_reg_count * sizeof(RATiedReg);
  }
...
public:
  ...
  Error add(
    ...
    if (use_id != Reg::kIdBad) {
      ...
      _used[group] |= axl::bit_mask<RegMask>(use_id);
      flags |= RATiedFlags::kUseFixed;
    }

    if (out_id != Reg::kIdBad) {
      _clobbered[group] |= axl::bit_mask<RegMask>(out_id);
    ...
    if (!tied_reg) {
      ...
      tied_reg->_rm_size = uint8_t(axl::max<uint32_t>(tied_reg->rm_size(), rm_size));
      return Error::kOk;
    }
  }
  ...
  Error add_call_arg(RAWorkReg* work_reg, uint32_t use_id) noexcept {
    ...
    RegGroup group = work_reg->group();
    RegMask allocable = axl::bit_mask<RegMask>(use_id);
  ...
  Error add_call_ret(RAWorkReg* work_reg, uint32_t out_id) noexcept {
    ...
    RegGroup group = work_reg->group();
    RegMask out_regs = axl::bit_mask<RegMask>(out_id);
    ...
    if (!tied_reg) {
      ...
      tied_reg = _cur++;
      tied_reg->init(work_reg, flags, axl::bit_ones<RegMask>, Reg::kIdBad, 0, out_regs, out_id, 0);
      work_reg->set_tied_reg(tied_reg);
...
#endif // ASMJIT_CORE_RA_INST_P_H_INCLUDED
