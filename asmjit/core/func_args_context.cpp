...
ASMJIT_FAVOR_SIZE Error FuncArgsContext::init_work_data(const FuncFrame& frame, const FuncArgsAssignment& args, const RAConstraints* constraints) noexcept {
  ...
  if (frame.has_preserved_fp()) {
    _work_data[size_t(RegGroup::kGp)]._arch_regs &= ~axl::bit_mask<RegMask>(arch_traits().fp_reg_id());
  }
  ...
  for (uint32_t arg_index = 0; arg_index < arg_count; arg_index++) {
    for (uint32_t value_index = 0; value_index < Globals::kMaxValuePack; value_index++) {
      ...
      if (dst.is_reg()) {
        ...
        if (ASMJIT_UNLIKELY(dst_id >= 32 || !axl::bit_test(dst_wd->arch_regs(), dst_id))) {
          return make_error(Error::kInvalidPhysId);
        }

        if (ASMJIT_UNLIKELY(axl::bit_test(dst_wd->dst_regs(), dst_id))) {
          return make_error(Error::kOverlappedRegs);
        }

        dst_wd->_dst_regs  |= axl::bit_mask<RegMask>(dst_id);
        dst_wd->_dst_shuf  |= axl::bit_mask<RegMask>(dst_id);
        dst_wd->_used_regs |= axl::bit_mask<RegMask>(dst_id);
      }
      else {
        if (!dst.has_type_id()) {
        ...
        _stack_dst_mask = uint8_t(_stack_dst_mask | axl::bit_mask<uint32_t>(signature.reg_group()));
      }
  ...
  if (sa_out_reg_id != Reg::kIdBad) {
    ...
    if (ASMJIT_UNLIKELY(axl::bit_test(gp_regs.dst_regs(), sa_out_reg_id))) {
      return make_error(Error::kOverlappedRegs);
    }
  ...
  if (sa_reg_required) {
    ...
    if (sa_cur_reg_id == Reg::kIdBad) {
      if (sa_out_reg_id != Reg::kIdBad && !gp_regs.is_assigned(sa_out_reg_id)) {
        ...
        sa_cur_reg_id = axl::ctz(available_regs);
      }
    }

    var.cur.init_reg(ptr_reg_type, sa_cur_reg_id, ptr_type_id);
    gp_regs.assign(var_id, sa_cur_reg_id);
    gp_regs._work_regs |= axl::bit_mask<RegMask>(sa_cur_reg_id);

    if (sa_out_reg_id != Reg::kIdBad) {
      var.out.init_reg(ptr_reg_type, sa_out_reg_id, ptr_type_id);
      gp_regs._dst_regs  |= axl::bit_mask<RegMask>(sa_out_reg_id);
      gp_regs._work_regs |= axl::bit_mask<RegMask>(sa_out_reg_id);
    }
    else {
      var.mark_done();
    }
    ...
    if (var.cur.is_reg() && var.out.is_reg()) {
      ...
      if (wd.is_assigned(dst_id)) {
        ...
        if (RegUtils::group_of(other.out.reg_type()) == group && other.out.reg_id() == src_id) {
          wd._num_swaps++;
          _reg_swaps_mask = uint8_t(_reg_swaps_mask | axl::bit_mask<uint32_t>(group));
        }
      }
    }
  }
...
ASMJIT_FAVOR_SIZE Error FuncArgsContext::mark_scratch_regs(FuncFrame& frame) noexcept {
  ...
  group_mask |= _reg_swaps_mask & ~axl::bit_mask<uint32_t>(RegGroup::kGp);
  ...
  for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
    if (axl::bit_test(group_mask, group)) {
      WorkData& wd = _work_data[group];
      if (wd._needs_scratch) {
        ...
        RegMask reg_mask = axl::extract_trailing_bit(regs);
