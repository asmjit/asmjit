...
//! \{
  ...
  #define ASMJIT_HAS_HOST_BACKEND __detected_at_runtime__
    ...
    #define ASMJIT_TARGET_ARCH_X86 64
  #elif defined(_M_IX86) || defined(__X86__) || defined(__i386__)
    #define ASMJIT_TARGET_ARCH_X86 32
  #else
    #define ASMJIT_TARGET_ARCH_X86 0
  #endif

  #if defined(_M_ARM64) || defined(__arm64__) || defined(__aarch64__)
  # define ASMJIT_TARGET_ARCH_ARM 64
  #elif defined(_M_ARM) || defined(_M_ARMT) || defined(__arm__) || defined(__thumb__) || defined(__thumb2__)
    #define ASMJIT_TARGET_ARCH_ARM 32
  #else
    #define ASMJIT_TARGET_ARCH_ARM 0
  #endif

  #if defined(_MIPS_ARCH_MIPS64) || defined(__mips64)
    #define ASMJIT_TARGET_ARCH_MIPS 64
  #elif defined(_MIPS_ARCH_MIPS32) || defined(_M_MRX000) || defined(__mips__)
    #define ASMJIT_TARGET_ARCH_MIPS 32
  #else
    #define ASMJIT_TARGET_ARCH_MIPS 0
  #endif
    ...
    #define ASMJIT_TARGET_ARCH_RISCV __riscv_xlen
  #else
    #define ASMJIT_TARGET_ARCH_RISCV 0
  #endif

  #if defined(__loongarch__) && defined(__loongarch_grlen)
    #define ASMJIT_TARGET_ARCH_LA __loongarch_grlen
  #else
    #define ASMJIT_TARGET_ARCH_LA 0
  #endif

  #if defined(__wasm64__)
    #define ASMJIT_TARGET_ARCH_WASM 64
  #elif defined(__wasm32__) || defined(__wasm__)
    #define ASMJIT_TARGET_ARCH_WASM 32
  #else
    #define ASMJIT_TARGET_ARCH_WASM 0
  #endif

  #define ASMJIT_TARGET_ARCH_BITS (ASMJIT_TARGET_ARCH_X86 | ASMJIT_TARGET_ARCH_ARM | ASMJIT_TARGET_ARCH_MIPS | ASMJIT_TARGET_ARCH_RISCV | ASMJIT_TARGET_ARCH_LA | ASMJIT_TARGET_ARCH_WASM)
  #if ASMJIT_TARGET_ARCH_BITS == 0 && !defined(_DOXYGEN)
    #undef ASMJIT_TARGET_ARCH_BITS
    #if defined(__LP64__) || defined(_LP64)
      #define ASMJIT_TARGET_ARCH_BITS 64
    #else
      #define ASMJIT_TARGET_ARCH_BITS 32
    #endif
  #endif

  #if defined(ASMJIT_NO_FOREIGN)
    #if !ASMJIT_TARGET_ARCH_X86 && !defined(ASMJIT_NO_X86)
      #define ASMJIT_NO_X86
    #endif

    #if ASMJIT_TARGET_ARCH_ARM != 32 && !defined(ASMJIT_NO_AARCH32)
      #define ASMJIT_NO_AARCH32
    #endif
    ...
    #elif !defined(ASMJIT_NO_AARCH64) && ASMJIT_TARGET_ARCH_ARM == 64
...
// Calling conventions.
#if ASMJIT_TARGET_ARCH_X86 == 32 && defined(__GNUC__)
...
#elif ASMJIT_TARGET_ARCH_X86 == 32 && defined(_MSC_VER)
...
#if ASMJIT_TARGET_ARCH_X86 && defined(_WIN32) && defined(_MSC_VER)
  #define ASMJIT_VECTORCALL __vectorcall
#elif ASMJIT_TARGET_ARCH_X86 && defined(_WIN32)
...
#if defined(__clang__)
  #define ASMJIT_NOUNROLL _Pragma("nounroll")
#elif defined(__GNUC__)
  // NOTE: We could use `_Pragma("GCC unroll 1")`, however, this doesn't apply to all loops and GCC emits many
  // of warnings such as "ignoring loop annotation", which cannot be turned off globally nor locally. So we
  // disable loop annotations when compiling with GCC. This comment is here so we can re-enable in the future.
  #define ASMJIT_NOUNROLL
#else
  #define ASMJIT_NOUNROLL
#endif
...
ASMJIT_INLINE_NODEBUG void* operator_new(size_t n) noexcept { return malloc(n); }
ASMJIT_INLINE_NODEBUG void operator_delete(void* p) noexcept { if (p) free(p); }
ASMJIT_END_SUB_NAMESPACE

#define ASMJIT_BASE_CLASS(TYPE)                                                                        \
  ASMJIT_INLINE void* operator new(size_t n) noexcept { return axl::operator_new(n); }                 \
  ASMJIT_INLINE void operator delete(void* ptr) noexcept { axl::operator_delete(ptr); }                \
                                                                                                       \
  ASMJIT_INLINE void* operator new(size_t, void* ptr) noexcept { return ptr; }                         \
  ASMJIT_INLINE void operator delete(void*, void*) noexcept {}                                         \
                                                                                                       \
  ASMJIT_INLINE void* operator new(size_t, asmjit::axl::PlacementNew ptr) noexcept { return ptr.ptr; } \
  ASMJIT_INLINE void operator delete(void*, asmjit::axl::PlacementNew) noexcept {}
#else
#define ASMJIT_BASE_CLASS(TYPE)
#endif
