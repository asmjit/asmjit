...
#ifndef ASMJIT_CORE_CPU_INFO_H_INCLUDED
#define ASMJIT_CORE_CPU_INFO_H_INCLUDED
...
public:
  ...
  static inline constexpr uint32_t kNumBitWords = kMaxFeatures / axl::bit_size_of<axl::BitWord>;
  //! \endcond
  ...
  //! Inline bit-array used as a storage for feature bits.
  using BitArray = axl::InplaceBitSet<kMaxFeatures>;
  //! Iterator that can iterate all CPU features set.
  using Iterator = BitArray::Iterator;
  ...
  struct Data {
    ...
    ASMJIT_INLINE_CONSTEXPR Data() noexcept = default;
    ASMJIT_INLINE_CONSTEXPR Data(const Data& other) noexcept = default;

    //! \name Equality
    //! \{

    //! Tests whether this CPU features data matches `other`.
    ASMJIT_INLINE_CONSTEXPR bool equals(const Data& other) const noexcept { return _bit_array.equals(other._bit_array); }
    ...
    ASMJIT_INLINE_CONSTEXPR Data& operator=(const Data& other) noexcept = default;

    [[nodiscard]]
    ASMJIT_INLINE_CONSTEXPR bool operator==(const Data& other) const noexcept { return equals(other); }
    ...
    [[nodiscard]]
    ASMJIT_INLINE_CONSTEXPR bool is_empty() const noexcept { return _bit_array.is_empty(); }

    //! Returns all features as array of bitwords (const).
    [[nodiscard]]
    ASMJIT_INLINE_NODEBUG const BitWord* bits() const noexcept { return _bit_array.data(); }

    //! Returns all features as array of bitwords (see \ref axl::BitWord).
    [[nodiscard]]
    ASMJIT_INLINE_NODEBUG BitWord* bits() noexcept { return _bit_array.data_mut(); }

    //! Returns the number of BitWords returned by \ref bits().
    [[nodiscard]]
    ASMJIT_INLINE_CONSTEXPR size_t bit_word_count() const noexcept { return _bit_array.bit_word_count(); }

    //! Returns \ref axl::BitVectorIterator, that can be used to iterate over all features efficiently.
    [[nodiscard]]
    ASMJIT_INLINE_NODEBUG Iterator iterator() const noexcept { return _bit_array.iterator(); }

    //! Tests whether the feature `feature_id` is present.
    template<typename FeatureId>
    [[nodiscard]]
    ASMJIT_INLINE_NODEBUG bool has(const FeatureId& feature_id) const noexcept { return _bit_array.has(feature_id); }
    ...
    [[nodiscard]]
    ASMJIT_INLINE_NODEBUG bool has_any(Args&&... other_feature_ids) const noexcept { return _bit_array.has_any(axl::forward<Args>(other_feature_ids)...); }

    //! Tests whether all features as defined by `other` are present.
    [[nodiscard]]
    ASMJIT_INLINE_NODEBUG bool has_all(const Data& other) const noexcept { return _bit_array.has_all(other._bit_array); }
    ...
    ASMJIT_INLINE_NODEBUG void reset() noexcept { _bit_array.reset(); }

    //! Adds the given CPU `feature_ids` to the list of features.
    //!
    //! \note This is a variadic argument function, which accepts multiple features.
    template<typename... Args>
    inline void add(Args&&... feature_ids) noexcept { _bit_array.set(axl::forward<Args>(feature_ids)...); }

    //! Adds the given CPU `feature_ids` to the list of features only if the given condition (bool) is true.
    //!
    //! \note This function is mostly used as a shorthand in CPU features detection.
    template<typename... Args>
    inline void add_if(bool condition, Args&&... feature_ids) noexcept { _bit_array.set_if(condition, axl::forward<Args>(feature_ids)...); }

    //! Removes the given CPU `feature_ids` from the list of features.
    template<typename... Args>
    inline void remove(Args&&... feature_ids) noexcept { _bit_array.clear(axl::forward<Args>(feature_ids)...); }

    template<typename Operator>
    ASMJIT_INLINE void combine(const Data& other) noexcept { _bit_array.template combine<Operator>(other._bit_array); }
  ...
  ASMJIT_INLINE_NODEBUG explicit CpuFeatures(const Data& data) noexcept : _data(data) {}
  ...
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG bool has_any(Args&&... args) const noexcept { return _data.has_any(axl::forward<Args>(args)...); }
  ...
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG void add(Args&&... args) noexcept { return _data.add(axl::forward<Args>(args)...); }

  //! Adds the given CPU `feature_id` to the list of features if `condition` is true.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG void add_if(bool condition, Args&&... args) noexcept { return _data.add_if(condition, axl::forward<Args>(args)...); }

  //! Removes the given CPU `feature_id` from the list of features.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG void remove(Args&&... args) noexcept { return _data.remove(axl::forward<Args>(args)...); }
  ...
  template<typename Operator>
  ASMJIT_INLINE void combine(const CpuFeatures& other) noexcept { _data.combine<Operator>(other._data); }
...
public:
  ...
  //! CPU vendor string.
  axl::InplaceString<15> _vendor {};
  //! CPU brand string.
  axl::InplaceString<63> _brand {};
  //! CPU features.
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG const char* vendor() const noexcept { return _vendor.data(); }

  //! Returns a CPU vendor string as span.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<const char> vendor_span() const noexcept { return _vendor.span(); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG const char* brand() const noexcept { return _brand.data(); }

  //! Returns a CPU brand string as span.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<const char> brand_span() const noexcept { return _brand.span(); }
  ...
  //! Adds the given CPU `feature_id` to the list of features.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG void add_feature(Args&&... args) noexcept { return _features.add(axl::forward<Args>(args)...); }

  //! Removes the given CPU `feature_id` from the list of features.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG void remove_feature(Args&&... args) noexcept { return _features.remove(axl::forward<Args>(args)...); }
...
#endif // ASMJIT_CORE_CPU_INFO_H_INCLUDED
