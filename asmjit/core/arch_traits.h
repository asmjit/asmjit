...
#ifndef ASMJIT_CORE_ARCH_TRAITS_H_INCLUDED
#define ASMJIT_CORE_ARCH_TRAITS_H_INCLUDED
...
#else
    ASMJIT_TARGET_ARCH_X86 == 32 ? kX86 :
    ASMJIT_TARGET_ARCH_X86 == 64 ? kX64 :

    ASMJIT_TARGET_ARCH_RISCV == 32 ? kRISCV32 :
    ASMJIT_TARGET_ARCH_RISCV == 64 ? kRISCV64 :

    ASMJIT_TARGET_ARCH_LA == 64 ? kLA64 :

    ASMJIT_TARGET_ARCH_ARM == 32 && axl::kIsLittleEndian ? kARM :
    ASMJIT_TARGET_ARCH_ARM == 32 && axl::kIsBigEndian    ? kARM_BE :
    ASMJIT_TARGET_ARCH_ARM == 64 && axl::kIsLittleEndian ? kAArch64 :
    ASMJIT_TARGET_ARCH_ARM == 64 && axl::kIsBigEndian    ? kAArch64_BE :

    ASMJIT_TARGET_ARCH_MIPS == 32 && axl::kIsLittleEndian ? kMIPS32_LE :
    ASMJIT_TARGET_ARCH_MIPS == 32 && axl::kIsBigEndian    ? kMIPS32_BE :
    ASMJIT_TARGET_ARCH_MIPS == 64 && axl::kIsLittleEndian ? kMIPS64_LE :
    ASMJIT_TARGET_ARCH_MIPS == 64 && axl::kIsBigEndian    ? kMIPS64_BE :
...
struct ArchTraits {
  ...
  axl::InplaceArray<InstHints, Globals::kNumVirtGroups> _inst_hints;

  //! Maps scalar TypeId values (from TypeId::_kIdBaseStart) to register types, see \ref TypeId.
  axl::InplaceArray<RegType, 32> _type_id_to_reg_type;
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_inst_hint(RegGroup group, InstHints feature) const noexcept { return axl::test(_inst_hints[group], feature); }
  ...
  ASMJIT_INLINE_NODEBUG bool has_reg_type(RegType type) const noexcept {
    ...
    return axl::bit_test(_supported_reg_types, uint32_t(type));
  }
...
#endif // ASMJIT_CORE_ARCH_TRAITS_H_INCLUDED
