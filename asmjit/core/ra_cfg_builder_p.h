...
#ifndef ASMJIT_CORE_RA_CFG_BUILDER_P_H_INCLUDED
#define ASMJIT_CORE_RA_CFG_BUILDER_P_H_INCLUDED
...
public:
  ...
  axl::ArenaVector<uint32_t> _shared_assignments_map {};
  ...
  Error run() noexcept {
    ...
    RAInstBuilder ib;
    axl::ArenaVector<RABlock*> blocks_with_unknown_jumps;
        ...
        if (cf == InstControlFlow::kJump) {
          ...
          for (RATiedReg& tied_reg : ib) {
            ...
            if (work_reg->group() == RegGroup::kGp) {
              ...
              _cur_block->add_exit_scratch_gp_regs(axl::bit_mask<RegMask>(use_id));
            }
          }
        }
  ...
  inline void log(const char* fmt, Args&&... args) noexcept {
    if (_logger) {
      _logger->logf(fmt, axl::forward<Args>(args)...);
    }
  }
  ...
  inline void log(const char* fmt, Args&&... args) noexcept {
    ...
    axl::maybe_unused(axl::forward<Args>(args)...);
  }

  inline void log_block(RABlock* block, uint32_t indentation = 0) noexcept {
    axl::maybe_unused(block, indentation);
  }

  inline void log_node(BaseNode* node, uint32_t indentation = 0, const char* action = nullptr) noexcept {
    axl::maybe_unused(node, indentation, action);
  }
#endif
...
#endif // ASMJIT_CORE_RA_CFG_BUILDER_P_H_INCLUDED
