...
// Required by `__cpuidex()` and `_xgetbv()`.
  ...
  #if ASMJIT_TARGET_ARCH_ARM >= 64 && defined(__GNUC__) && defined(__linux__) && 0
  ...
  #if ASMJIT_TARGET_ARCH_ARM >= 64 && defined(__OpenBSD__)
    #include <machine/cpu.h>
    #include <sys/sysctl.h>
  #endif

  #if ASMJIT_TARGET_ARCH_ARM >= 64 && defined(__NetBSD__)
    #include <sys/sysctl.h>
  #endif

#endif // ASMJIT_TARGET_ARCH_ARM

#if !defined(_WIN32) && (ASMJIT_TARGET_ARCH_X86 || ASMJIT_TARGET_ARCH_ARM)
  #include <unistd.h>
#endif
...
#elif defined(__GNUC__) && ASMJIT_TARGET_ARCH_X86 == 32
  __asm__ __volatile__(
    "mov %%ebx, %%edi\n"
    "cpuid\n"
    "xchg %%edi, %%ebx\n" : "=a"(out->eax), "=D"(out->ebx), "=c"(out->ecx), "=d"(out->edx) : "a"(in_eax), "c"(in_ecx));
#elif defined(__GNUC__) && ASMJIT_TARGET_ARCH_X86 == 64
  __asm__ __volatile__(
    "mov %%rbx, %%rdi\n"
    "cpuid\n"
    "xchg %%rdi, %%rbx\n" : "=a"(out->eax), "=D"(out->ebx), "=c"(out->ecx), "=d"(out->edx) : "a"(in_eax), "c"(in_ecx));
...
static inline void simplify_cpu_vendor(CpuInfo& cpu, uint32_t d0, uint32_t d1, uint32_t d2) noexcept {
  ...
  cpu._vendor.assign(Span<const char>(table[i].normalized, strlen(table[i].normalized)));
}

static ASMJIT_FAVOR_SIZE size_t simplify_cpu_brand(char* s) noexcept {
  char* begin = s;
  char* d = s;
...
static ASMJIT_FAVOR_SIZE void detect_x86_cpu(CpuInfo& cpu) noexcept {
  ...
  if (max_sub_leaf_id_0x7 >= 1) {
    ...
    if (features.has_avx512_f()) {
      avx10_enabled = axl::bit_test(regs.edx, 19);
    }
  ...
  char* brand_str = cpu._brand.data_mut();
  char* brand_str_ptr = brand_str;

  do {
    cpuid_query(&regs, i);
    switch (i) {
      case 0x80000000u:
        max_id = axl::min<uint32_t>(regs.eax, kHighestProcessedEAX);
        break;
      ...
      case 0x80000003u:
      case 0x80000004u:
        axl::storea_u32(brand_str_ptr +  0, regs.eax);
        axl::storea_u32(brand_str_ptr +  4, regs.ebx);
        axl::storea_u32(brand_str_ptr +  8, regs.ecx);
        axl::storea_u32(brand_str_ptr + 12, regs.edx);
        brand_str_ptr += 16;
  ...
  cpu._brand._assign_size(simplify_cpu_brand(brand_str));
}
...
#endif // ASMJIT_TARGET_ARCH_ARM == 32
...
#endif // ASMJIT_TARGET_ARCH_ARM == 64
...
static ASMJIT_FAVOR_SIZE void merge_hw_caps(CpuInfo& cpu, const Mask& mask, const Map& map) noexcept {
  ...
  axl::BitWordIterator<Mask> it(mask);
  while (it.has_next()) {
...
#elif defined(__NetBSD__) && ASMJIT_TARGET_ARCH_ARM >= 64
...
#elif defined(__OpenBSD__) && ASMJIT_TARGET_ARCH_ARM >= 64
...
static ASMJIT_FAVOR_SIZE void detect_arm_cpu(CpuInfo& cpu) noexcept {
  ...
  invoke_sysctl_by_name<char>("machdep.cpu.brand_string", cpu._brand.data_mut(), cpu._brand.capacity());

  cpu._brand._assign_size(strlen(cpu._brand.data()));
  cpu._vendor.assign(Span<const char>("APPLE", 5));
...
static ASMJIT_FAVOR_SIZE CpuHints recalculate_hints(const CpuInfo& cpu_info, const CpuFeatures::ARM& features) noexcept {
  axl::maybe_unused(cpu_info, features);
...
const CpuInfo& CpuInfo::host() noexcept {
  ...
  static CpuInfo cpu_info_global;
  ...
  if (!cpu_info_initialized_flag.load(std::memory_order::relaxed)) {
    ...
    cpu_info_initialized_flag.store(1, std::memory_order::seq_cst);
  }
...
#else
  axl::maybe_unused(info, features);
