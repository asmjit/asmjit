...
bool CodeWriterUtils::encode_offset32(uint32_t* dst, int64_t offset64, const OffsetFormat& format) noexcept {
  ...
  if (unsigned_logic) {
    if (discard_lsb) {
      ...
      if ((offset64 & axl::trailing_bits_run<uint32_t>(discard_lsb)) != 0) {
        return false;
      }
      offset64 = int64_t(uint64_t(offset64) >> discard_lsb);
    }

    value = uint32_t(offset64 & axl::trailing_bits_run<uint32_t>(bit_count));
    if (value != offset64) {
      return false;
    }
  }
  ...
  switch (format.type()) {
    ...
    case OffsetType::kUnsignedOffset: {
      *dst = (value & axl::trailing_bits_run<uint32_t>(bit_count)) << bit_shift;
      return true;
    }
    ...
    case OffsetType::kAArch32_ADR: {
      ...
      *dst = (axl::bit_mask<uint32_t>(22) << u) | (encoded_imm << bit_shift);
      return true;
    }
    ...
    case OffsetType::kAArch64_ADRP: {
      ...
      uint32_t imm_hi = (value >> 2) & axl::trailing_bits_run<uint32_t>(19);
...
bool CodeWriterUtils::encode_offset64(uint64_t* dst, int64_t offset64, const OffsetFormat& format) noexcept {
  ...
  if (format.type() == OffsetType::kUnsignedOffset) {
    ...
    value = uint64_t(offset64) & axl::trailing_bits_run<uint64_t>(bit_count);
  ...
  switch (format.type()) {
    case OffsetType::kSignedOffset:
    case OffsetType::kUnsignedOffset: {
      *dst = (value & axl::trailing_bits_run<uint64_t>(bit_count)) << format.imm_bit_shift();
      return true;
    }
...
bool CodeWriterUtils::write_offset(void* dst, int64_t offset64, const OffsetFormat& format) noexcept {
  ...
  switch (format.value_size()) {
    case 1: {
      ...
      axl::store_u8(dst, uint8_t(axl::load_u8(dst) | mask));
      return true;
    }
    ...
    case 2: {
      ...
      axl::storeu_u16_le(dst, uint16_t(axl::loadu_u16_le(dst) | mask));
      return true;
    }
    ...
    case 4: {
      ...
      axl::storeu_u32_le(dst, axl::loadu_u32_le(dst) | mask);
      return true;
    }
    ...
    case 8: {
      ...
      axl::storeu_u64_le(dst, axl::loadu_u64_le(dst) | mask);
      return true;
    }
