...
#ifndef ASMJIT_CORE_CODE_HOLDER_H_INCLUDED
#define ASMJIT_CORE_CODE_HOLDER_H_INCLUDED
...
public:
  ...
  uint32_t _section_id = Globals::kInvalidId;
...
public:
  ...
  uint64_t _offset = Globals::kNoSectionOffset;
  ...
  axl::InplaceString<Globals::kMaxSectionNameSize + 1> _name {};
  //! Code or data buffer.
  CodeBuffer _buffer {};
  ...
  ASMJIT_INLINE_NODEBUG Section(Span<const char> section_name, uint32_t section_id, SectionFlags flags, uint64_t offset, uint32_t alignment, int order) noexcept
    : SectionOrLabelEntryExtraHeader{section_id, LabelType::kAnonymous, LabelFlags::kNone, uint16_t(flags)},
      _alignment(alignment),
      _order(order),
      _offset(offset),
      _name(section_name) {}

  ASMJIT_INLINE_NODEBUG Section(const Section& other) noexcept = default;
  ...
  ASMJIT_INLINE_NODEBUG Section& operator=(const Section& other) noexcept = default;
  ...
  //! Returns the section name, as a null terminated string.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<const char> name_span() const noexcept { return _name.span(); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_flag(SectionFlags flag) const noexcept { return axl::test(_internal_uint16_data, uint32_t(flag)); }
  ...
  //! Returns an identifier that can be used to compare multiple sections for sorting purposes. Each section would return
  //! a unique id.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG int64_t comparable_id() const noexcept { return (int64_t(_order) << 32) | int64_t(_section_id); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG uint64_t real_size() const noexcept { return axl::max<uint64_t>(virtual_size(), buffer_size()); }
...
class AddressTableEntry : public axl::ArenaTreeNodeT<AddressTableEntry> {
public:
...
public:
  ...
  struct ExtraData : public SectionOrLabelEntryExtraHeader {
    ...
    uint32_t _parent_id = Globals::kInvalidId;
    //! Label name length.
    uint32_t _name_size = 0;
    ...
    ASMJIT_INLINE_NODEBUG const char* name() const noexcept { return axl::offset_ptr<char>(this, sizeof(ExtraData)); }
  };
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_label_flag(LabelFlags flag) const noexcept { return axl::test(_object_data->_internal_label_flags, flag); }
  ...
  ASMJIT_INLINE_NODEBUG bool has_fixups() const noexcept {
    return axl::bool_and(!is_bound(), _offset_or_fixups != 0u);
  }
...
//!
//!   Fn fn1 = axl::ptr_as_func<Fn>(module, func1_offset);
//!   Fn fn2 = axl::ptr_as_func<Fn>(module, func2_offset);
//!
//!   printf("fn1()=%u fn2()=%u\n", fn1(), fn2());
//!
...
public:
  ...
  //! \cond INTERNAL
  struct NamedLabelExtraData : public axl::ArenaHashNode {
  ...
  //! Section entries.
  axl::ArenaVector<Section*> _sections;
  //! Section entries sorted by section order and then section id.
  axl::ArenaVector<Section*> _sections_by_order;

  //! Label entries.
  axl::ArenaVector<LabelEntry> _label_entries;
  //! Relocation entries.
  axl::ArenaVector<RelocEntry*> _relocations;
  //! Label name -> LabelEntry::ExtraData (only used by labels that have a name and are not anonymous).
  axl::ArenaHash<NamedLabelExtraData> _named_labels;
  ...
  axl::ArenaTree<AddressTableEntry> _address_table_entries;
  ...
  ASMJIT_API explicit CodeHolder(Span<uint8_t> inplace_storage = Span<uint8_t>{}) noexcept;

  //! Creates an uninitialized CodeHolder with inline storage.
  //!
  //! \note This is a convenience constructor that creates a memory span from `inplace_storage`
  //! and uses it to call a different constructor. The advantage of axl::InplaceStorage is that
  //! it guarantees a reasonable alignment of its underlying buffer. `InplaceStorage` cannot go out
  //! of scope before `CodeHolder`.
  template<size_t N>
  ASMJIT_INLINE explicit CodeHolder(axl::InplaceStorage<N>& inplace_storage) noexcept :
    CodeHolder(inplace_storage.span()) {}
  ...
  //! destructor.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG axl::Arena& arena() const noexcept { return const_cast<axl::Arena&>(_arena); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<Section*> sections() const noexcept { return _sections.span_mut(); }

  //! Returns an array of `Section*` records sorted according to section order first, then section id.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<Section*> sections_by_order() const noexcept { return _sections_by_order.span_mut(); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE Section* section_by_name(Span<const char> name) const noexcept { return section_by_name(name.data(), name.size()); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<const LabelEntry> label_entries() const noexcept { return _label_entries.span(); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<const RelocEntry*> reloc_entries() const noexcept { return _relocations.span(); }

  //! Returns a RelocEntry of the given `id`.
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG const RelocEntry* reloc_entry_of(uint32_t id) const noexcept { return _relocations[id]; }
...
#endif // ASMJIT_CORE_CODE_HOLDER_H_INCLUDED
