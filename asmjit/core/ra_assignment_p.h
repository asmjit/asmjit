...
#ifndef ASMJIT_CORE_RA_ASSIGNMENT_P_H_INCLUDED
#define ASMJIT_CORE_RA_ASSIGNMENT_P_H_INCLUDED
...
#include <asmjit/core/ra_reg_p.h>
...
public:
  ...
  struct Layout {
    ...
    const axl::ArenaVector<RAWorkReg*>* work_regs;
  ...
  struct PhysToWorkMap {
    ...
    static ASMJIT_INLINE_NODEBUG size_t size_of(size_t count) noexcept {
      return axl::Arena::aligned_size(sizeof(PhysToWorkMap) - sizeof(uint32_t) + count * sizeof(uint32_t));
    }
    ...
    ASMJIT_INLINE void unassign(RegGroup group, uint32_t phys_id, uint32_t index_in_work_ids) noexcept {
      assigned.clear(group, axl::bit_mask<RegMask>(phys_id));
      dirty.clear(group, axl::bit_mask<RegMask>(phys_id));
  ...
  struct WorkToPhysMap {
    ...
    [[nodiscard]]
    static ASMJIT_INLINE_NODEBUG size_t size_of(size_t count) noexcept {
      return axl::Arena::aligned_size(size_t(count) * sizeof(uint8_t));
    }
  ...
  axl::InplaceArray<RAWorkId*, Globals::kNumVirtGroups> _phys_to_work_ids;
  ...
  ASMJIT_INLINE void init_layout(const RARegCount& phys_count, const axl::ArenaVector<RAWorkReg*>& work_regs) noexcept {
  ...
  inline bool is_phys_assigned(RegGroup group, uint32_t phys_id) const noexcept {
    ASMJIT_ASSERT(phys_id < Globals::kMaxPhysRegs);
    return axl::bit_test(_phys_to_work_map->assigned[group], phys_id);
  }

  [[nodiscard]]
  inline bool is_phys_dirty(RegGroup group, uint32_t phys_id) const noexcept {
    ASMJIT_ASSERT(phys_id < Globals::kMaxPhysRegs);
    return axl::bit_test(_phys_to_work_map->dirty[group], phys_id);
  }
  ...
  inline void assign(RegGroup group, RAWorkId work_id, uint32_t phys_id, bool dirty) noexcept {
    ...
    RegMask reg_mask = axl::bit_mask<RegMask>(phys_id);
    _phys_to_work_map->assigned[group] |= reg_mask;
    _phys_to_work_map->dirty[group] |= reg_mask & axl::bool_as_mask<RegMask>(dirty);
  ...
  inline void reassign(RegGroup group, RAWorkId work_id, uint32_t dst_phys_id, uint32_t src_phys_id) noexcept {
    ...
    RegMask src_mask = axl::bit_mask<RegMask>(src_phys_id);
    RegMask dst_mask = axl::bit_mask<RegMask>(dst_phys_id);
  ...
  inline void swap(RegGroup group, RAWorkId a_work_id, uint32_t a_phys_id, RAWorkId b_work_id, uint32_t b_phys_id) noexcept {
    ...
    RegMask a_mask = axl::bit_mask<RegMask>(a_phys_id);
    RegMask b_mask = axl::bit_mask<RegMask>(b_phys_id);
    RegMask flip_mask = axl::bool_as_mask<RegMask>(((_phys_to_work_map->dirty[group] & a_mask) != 0) ^ ((_phys_to_work_map->dirty[group] & b_mask) != 0));
  ...
  ASMJIT_INLINE void swap(RAAssignment& other) noexcept {
    axl::swap(_work_to_phys_map, other._work_to_phys_map);
    axl::swap(_phys_to_work_map, other._phys_to_work_map);
  ...
  inline void assign_work_ids_from_phys_ids() noexcept {
    ...
    for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
      ...
      axl::BitWordIterator<RegMask> it(_phys_to_work_map->assigned[group]);
...
public:
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<const BitWord> live_in() const noexcept { return _live_in.span(); }
...
#endif // ASMJIT_CORE_RA_ASSIGNMENT_P_H_INCLUDED
