...
#ifndef ASMJIT_CORE_RA_PASS_P_H_INCLUDED
#define ASMJIT_CORE_RA_PASS_P_H_INCLUDED
...
public:
  ...
  axl::ArenaVector<RABlock*> _blocks {};
  //! Function exit blocks (usually one, but can contain more).
  axl::ArenaVector<RABlock*> _exits {};
  //! Post order view (POV).
  axl::ArenaVector<RABlock*> _pov {};
  ...
  axl::ArenaVector<RASharedAssignment> _shared_assignments {};
  ...
  axl::InplaceArray<uint8_t, 2> _scratch_reg_indexes {};
  ...
  axl::ArenaVector<RAWorkReg*> _work_regs;
  //! Work registers per register group.
  axl::InplaceArray<axl::ArenaVector<RAWorkReg*>, Globals::kNumVirtGroups> _work_regs_of_group;
  ...
  axl::InplaceArray<RAStrategy, Globals::kNumVirtGroups> _strategy;
  ...
  axl::InplaceArray<RALiveSpans*, Globals::kNumVirtGroups> _global_live_spans {};
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Logger* logger_if(DiagnosticOptions option) const noexcept { return axl::test(_diagnostic_options, option) ? _logger : nullptr; }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<RASharedAssignment> shared_assignments() const { return _shared_assignments.span_mut(); }
  ...
  inline void make_unavailable(RegGroup group, uint32_t reg_id) noexcept {
    _available_regs[group] &= ~axl::bit_mask<RegMask>(reg_id);
  }
  ...
  Error run_on_function(axl::Arena& arena, FuncNode* func, bool last) noexcept;
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG Span<RABlock*> blocks() noexcept { return _blocks.span_mut(); }
  ...
  ASMJIT_INLINE RAInst* new_ra_inst(InstRWFlags inst_rw_flags, RATiedFlags flags, uint32_t tied_reg_count, const RARegMask& clobbered_regs) noexcept {
    ...
    return new(axl::PlacementNew{p}) RAInst(inst_rw_flags, flags, tied_reg_count, clobbered_regs);
  }
  ...
  ASMJIT_INLINE Error assign_ra_inst(BaseNode* node, RABlock* block, RAInstBuilder& ib) noexcept {
    ...
    for (uint32_t i = 0; i < tied_reg_count; i++) {
      ...
      if (work_reg->_single_basic_block_id != block_id) {
        if (axl::bool_or(work_reg->_single_basic_block_id != kBadBlockId, !tied_reg->is_write_only())) {
      ...
      if (tied_reg->has_use_id()) {
        ...
        ra_inst->_used_regs[group] |= axl::bit_mask<RegMask>(tied_reg->use_id());
      }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG axl::ArenaVector<RAWorkReg*>& work_regs() noexcept { return _work_regs; }

  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG axl::ArenaVector<RAWorkReg*>& work_regs(RegGroup group) noexcept { return _work_regs_of_group[group]; }

  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG const axl::ArenaVector<RAWorkReg*>& work_regs() const noexcept { return _work_regs; }

  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG const axl::ArenaVector<RAWorkReg*>& work_regs(RegGroup group) const noexcept { return _work_regs_of_group[group]; }
  ...
  ASMJIT_INLINE Error rewrite_iterate(Lambda&& fn) noexcept {
    ...
    Span<RABlock*> pov = _pov.span_mut();
...
inline axl::Arena& RABlock::arena() const noexcept { return _ra->arena(); }
...
#endif // ASMJIT_CORE_RA_PASS_P_H_INCLUDED
