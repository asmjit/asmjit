...
#ifndef ASMJIT_CORE_RA_DEFS_P_H_INCLUDED
#define ASMJIT_CORE_RA_DEFS_P_H_INCLUDED
...
using RAWorkRegVector = axl::ArenaVector<RAWorkReg*>;
...
struct RAStrategy {
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_flag(RAStrategyFlags flag) const noexcept { return axl::test(_flags, flag); }
...
struct RARegMask {
  ...
  [[nodiscard]]
  inline bool is_empty() const noexcept { return !_masks.reduce<axl::Or>(); }

  [[nodiscard]]
  inline bool has(RegGroup group, RegMask mask = 0xFFFFFFFFu) const noexcept { return (_masks[group] & mask) != 0; }

  template<class Operator>
  inline void op(const RARegMask& other) noexcept { _masks.combine<Operator>(other._masks); }

  template<class Operator>
  inline void op(RegGroup group, RegMask mask) noexcept { _masks[group] = Operator::op(_masks[group], mask); }

  inline void clear(RegGroup group, RegMask mask) noexcept { _masks[group] = _masks[group] & ~mask; }

  inline void clear(const RegMasks& masks) noexcept { _masks.combine<axl::AndNot>(masks); }
...
public:
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_used(RegGroup group) const noexcept { return axl::bit_test(_packed, kIndexUsed + uint32_t(group)); }

  ASMJIT_INLINE_NODEBUG void make_used(RegGroup group) noexcept { _packed |= axl::bit_mask<uint32_t>(kIndexUsed + uint32_t(group)); }

  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_fixed() const noexcept { return (_packed & kMaskFixed) != 0u; }

  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_fixed(RegGroup group) const noexcept { return axl::bit_test(_packed, kIndexFixed + uint32_t(group)); }

  ASMJIT_INLINE_NODEBUG void make_fixed(RegGroup group) noexcept { _packed |= axl::bit_mask<uint32_t>(kIndexFixed + uint32_t(group)); }

  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_clobbered() const noexcept { return (_packed & kMaskClobbered) != 0u; }

  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_clobbered(RegGroup group) const noexcept { return axl::bit_test(_packed, kIndexClobbered + uint32_t(group)); }

  ASMJIT_INLINE_NODEBUG void make_clobbered(RegGroup group) noexcept { _packed |= axl::bit_mask<uint32_t>(kIndexClobbered + uint32_t(group)); }
...
public:
  ...
  axl::InplaceArray<uint32_t, Globals::kNumVirtGroups> n {};
...
public:
  ...
  axl::ArenaVector<RALiveSpan> _data {};
  ...
  ASMJIT_INLINE_NODEBUG void release(axl::Arena& arena) noexcept { _data.release(arena); }
  ...
  ASMJIT_INLINE Error open_at(axl::Arena& arena, NodePosition start, NodePosition end) noexcept {
    bool was_open;
    return open_at(arena, start, end, was_open);
  }

  ASMJIT_INLINE Error open_at(axl::Arena& arena, NodePosition start, NodePosition end, bool& was_open) noexcept {
    size_t size = _data.size();
    was_open = false;
  ...
  [[nodiscard]]
  ASMJIT_INLINE Error non_overlapping_union_of(axl::Arena& arena, const RALiveSpans& x, const RALiveSpans& y) noexcept {
...
struct RATiedReg {
  ...
  static inline RATiedFlags consecutive_data_to_flags(uint32_t offset) noexcept {
    ...
    constexpr uint32_t kOffsetShift = axl::ctz(RATiedFlags::kConsecutiveData);
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_flag(RATiedFlags flag) const noexcept { return axl::test(_flags, flag); }
...
#endif // ASMJIT_CORE_RA_DEFS_P_H_INCLUDED
