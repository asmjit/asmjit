...
#ifndef ASMJIT_CORE_VIRT_MEM_H_INCLUDED
#define ASMJIT_CORE_VIRT_MEM_H_INCLUDED
...
enum class MemoryFlags : uint32_t {
  ...
  //! Advise to use large pages.
  //!
  //! \remarks When enabled, `MADV_HUGEPAGE` will be used to advise using large pages. This option is also enabled by
  //! \ref JitAllocator when memory mapping with `kMMapLargePages` failed to at least advise using large pages (but
  //! not all platforms actually support the option).
  kMAdviseLargePages = 0x00000400u,
...
struct HardenedRuntimeInfo {
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_flag(HardenedRuntimeFlags flag) const noexcept { return axl::test(flags, flag); }
...
#endif
#endif // ASMJIT_CORE_VIRT_MEM_H_INCLUDED
