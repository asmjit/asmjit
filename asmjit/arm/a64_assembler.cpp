...
static inline SizeOp element_type_to_size_op(uint32_t vec_op_type, RegType reg_type, VecElementType element_type) noexcept {
  ...
  size_t index = (axl::min<uint32_t>(diff(reg_type, RegType::kVec8), diff(RegType::kVec128, RegType::kVec8) + 1) << 3) | uint32_t(element_type);
  SizeOp op = table.array[index];
  SizeOp modified_op { uint8_t(op.value & map.size_op_mask) };

  if (!axl::bit_test(map.accept_mask, op.value)) {
...
static inline bool check_mem_base_index_rel(const Mem& mem) noexcept {
  ...
  if (!axl::bit_test(kBaseMask, base_type)) {
    return false;
  }

  if (base_type > RegType::kLabelTag) {
    // Index allows either Gp32 or Gp64.
    if (!axl::bit_test(kIndexMask, index_type)) {
      return false;
    }
...
static inline bool pick_fp_opcode(const Vec& reg, uint32_t s_op, uint32_t s_hf, uint32_t v_op, uint32_t v_hf, Opcode* opcode, uint32_t* sz_out) noexcept {
  ...
  if (!reg.has_element_type()) {
    ...
    if (sz > 2u || !axl::bit_test(sz_bits_table[s_hf].size_mask, sz)) {
      return false;
    }
    ...
    *sz_out = sz;
    ...
    if (q > 1u || sz > 2u || !axl::bit_test(sz_bits_table[v_hf].size_mask, sz)) {
      return false;
    }
...
static inline bool check_gp_type(const Operand_& op, uint32_t allowed) noexcept {
  ...
  return axl::bit_test(allowed << uint32_t(RegType::kGp32), type);
}
...
static const axl::InplaceArray<uint8_t, 32> common_hi_reg_id_of_type_table = {{
  ...
  switch (inst_info->_encoding) {
    ...
    case InstDB::kEncodingBaseOpImm: {
      ...
      if (isign4 == ENC_OPS1(Imm)) {
        ...
        uint32_t imm_max = 1u << op_data.imm_bits;
    ...
    case InstDB::kEncodingBaseRRII: {
      ...
      if (isign4 == ENC_OPS4(Reg, Reg, Imm, Imm)) {
        ...
        if (o2.as<Imm>().value_as<uint64_t>() >= axl::bit_mask<uint32_t>(op_data.a_imm_size + op_data.a_imm_discard_lsb) ||
            o3.as<Imm>().value_as<uint64_t>() >= axl::bit_mask<uint32_t>(op_data.b_imm_size + op_data.b_imm_discard_lsb))
          goto InvalidImmediate;
    ...
    case InstDB::kEncodingBaseMovKNZ: {
      ...
      if (isign4 == ENC_OPS3(Reg, Imm, Imm)) {
        ...
        if (imm16 > 0xFFFFu || shift_value > 48 || shift_type != uint32_t(ShiftOp::kLSL))
          goto InvalidImmediate;

        // Convert shift value to 'hw' field.
        uint32_t hw = uint32_t(shift_value) >> 4;
        if ((hw << 4) != uint32_t(shift_value))
          goto InvalidImmediate;
    ...
    case InstDB::kEncodingBaseLogical: {
      ...
      if (isign4 == ENC_OPS3(Reg, Reg, Imm) && op_data.immediate_op != 0) {
        ...
        uint64_t imm_mask = axl::trailing_bits_run<uint64_t>(op_size);
        ...
        bool is_ands = (opcode.get() & kOpANDS) == kOpANDS;

        if (!check_gp_id(o0, is_ands ? kZR : kSP) || !check_gp_id(o1, kZR))
          goto InvalidPhysId;
    ...
    case InstDB::kEncodingBaseBfc: {
      ...
      if (isign4 == ENC_OPS3(Reg, Imm, Imm)) {
        ...
        uint32_t lsb32 = axl::neg(uint32_t(lsb)) & (op_size - 1);
    ...
    case InstDB::kEncodingBaseBfi: {
      ...
      if (isign4 == ENC_OPS4(Reg, Reg, Imm, Imm)) {
        ...
        uint32_t imm_l = axl::neg(uint32_t(lsb)) & (op_size - 1);
    ...
    case InstDB::kEncodingBaseShift: {
      ...
      if (isign4 == ENC_OPS3(Reg, Reg, Imm) && op_data.immediate_op()) {
        ...
        if (op_data.ror == 0) {
          ...
          uint32_t ubfm_imm_r = axl::neg(uint32_t(imm_r)) & (op_size - 1);
    ...
    case InstDB::kEncodingBaseMinMax: {
      ...
      if (isign4 == ENC_OPS3(Reg, Reg, Imm)) {
        ...
        if (op_data.immediate_op & (1u << 18)) {
          // Zero extend imm.
          if (!axl::is_uint_n<8>(imm)) {
            goto InvalidImmediate;
          }
        }
        else {
          // Sign extend imm.
          if (!axl::is_int_n<8>(int64_t(imm))) {
            goto InvalidImmediate;
          }
        }
    ...
    case InstDB::kEncodingBaseBranchRel: {
      ...
      if (isign4 == ENC_OPS1(Label) || isign4 == ENC_OPS1(Imm)) {
        ...
        if (inst_cc != CondCode::kAL || axl::bit_test(opcode.v, 30)) {
          if (opcode.has_x()) {
    ...
    case InstDB::kEncodingBasePrfm: {
      ...
      if (isign4 == ENC_OPS2(Imm, Mem)) {
        ...
        if (m.has_base_reg()) {
          ...
          if (axl::is_uint_n<12>(imm12) && (imm12 << imm_shift) == uint32_t(offset32)) {
...
Case_BaseLdurStur:
      ...
      if (isign4 == ENC_OPS2(Reg, Mem)) {
        ...
        if (m.has_base_reg() && !m.has_index()) {
          ...
          if (axl::shl(offset32, op_data.imm_shift) != m.offset_lo32())
            goto InvalidDisplacement;
    ...
    case InstDB::kEncodingSimdFcvtSV: {
      ...
      if (isign4 == ENC_OPS3(Reg, Reg, Imm) && op_data.is_fixed_point()) {
        ...
        if (o0.as<Reg>().is_vec() && o1.as<Reg>().is_vec()) {
          ...
          uint32_t imm = axl::neg(scale) & axl::trailing_bits_run<uint32_t>(sz + 4 + 1);
    ...
    case InstDB::kEncodingFSimdPair: {
      ...
      if (isign4 == ENC_OPS2(Reg, Reg)) {
        ...
        static const uint32_t sz_signatures[3] = {
        ...
        if (o1.signature() != sz_signatures[sz])
          goto InvalidInstruction;
    ...
    case InstDB::kEncodingSimdDup: SimdDup: {
      if (isign4 == ENC_OPS2(Reg, Reg)) {
        ...
        if (o1.as<Reg>().is_gp()) {
          ...
          if (q > 1 || !axl::bit_test(kValidEncodings, (q << 3) | element_type))
            goto InvalidInstruction;
      ...
      if (isign4 == ENC_OPS2(Reg, Mem)) {
        ...
        if (m.has_base_reg()) {
          ...
          if (m.is_pre_or_post()) {
            ...
            // If this instruction is not encodable with scaled unsigned offset, try unscaled signed offset.
            if (!axl::is_uint_n<12>(imm12) || (imm12 << xsz) != uint32_t(offset32)) {
              inst_id = op_data.u_alt_inst_id;
              inst_info = &InstDB::_inst_info_table[inst_id];
              encoding_index = inst_info->_encoding_data_index;
    ...
    case InstDB::kEncodingSimdLdpStp: {
      ...
      if (isign4 == ENC_OPS3(Reg, Reg, Mem)) {
        ...
        // Make sure we didn't lose bits by applying the mandatory offset shift.
        if (axl::shl(offset32, offset_shift) != m.offset_lo32())
          goto InvalidDisplacement;
...
EmitOp_DispImm:
  {
    if ((offset_value & axl::trailing_bits_run<uint32_t>(offset_format.imm_discard_lsb())) != 0) {
      goto InvalidDisplacement;
    }
    ...
    uint32_t disp_imm32 = uint32_t(disp_imm64 & axl::trailing_bits_run<uint32_t>(offset_format.imm_bit_count()));
