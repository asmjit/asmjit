...
Error FormatterInternal::format_feature(String& sb, uint32_t feature_id) noexcept {
  ...
  static constexpr auto feature_string_index = axl::make_string_index<size_t(CpuFeatures::ARM::kMaxValue) + 2u>(feature_string_data);
...
ASMJIT_FAVOR_SIZE Error FormatterInternal::format_cond_code(String& sb, CondCode cc) noexcept {
  static const char cond_code_string_data[] =
    ...
    "<Unknown>";
  return sb.append(cond_code_string_data + axl::min<uint32_t>(uint32_t(cc), 16u) * 3);
}
...
ASMJIT_FAVOR_SIZE Error FormatterInternal::format_register(
...
#else
  axl::maybe_unused(emitter, flags);
#endif
  ...
  if (element_type) {
    element_type = axl::min(element_type, kElementTypeCount);
...
ASMJIT_FAVOR_SIZE Error FormatterInternal::format_register_list(
  ...
  ASMJIT_PROPAGATE(sb.append('{'));
  while (reg_mask != 0u) {
    uint32_t start = axl::ctz(reg_mask);
    uint32_t count = 0u;
...
ASMJIT_FAVOR_SIZE Error FormatterInternal::format_operand(
  ...
  if (op.is_mem()) {
    ...
    if (m.has_offset()) {
      ...
      if (axl::test(flags, FormatFlags::kHexOffsets) && uint64_t(off) > 9) {
        base = 16;
      }
  ...
  if (op.is_imm()) {
    ...
    if (axl::test(flags, FormatFlags::kHexImms) && uint64_t(val) > 9) {
      ASMJIT_PROPAGATE(sb.append("0x"));
