...
#if defined(ASMJIT_TEST) && !defined(ASMJIT_NO_AARCH64)
...
TEST_CASE(a64_operand) {
  TEST_LOG("Checking if a64::reg(...) matches built-in IDs");
  EXPECT_EQ(w(5), w5);
  EXPECT_EQ(x(5), x5);

  TEST_LOG("Checking Gp register properties");
  ...
  TEST_LOG("Checking Vec register properties");
  ...
  TEST_LOG("Checking Vec register element[] access");
  Vec vd_1 = v15.d(1);
...
#endif // ASMJIT_TEST && !ASMJIT_NO_AARCH64
