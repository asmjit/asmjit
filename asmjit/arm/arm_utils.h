...
#ifndef ASMJIT_ARM_ARM_UTILS_H_INCLUDED
#define ASMJIT_ARM_ARM_UTILS_H_INCLUDED
...
static ASMJIT_INLINE bool encode_logical_imm(uint64_t imm, uint32_t width, Out<LogicalImm> out) noexcept {
  ...
  uint64_t lsb_mask = axl::trailing_bits_run<uint64_t>(width);
  imm &= lsb_mask;
  ...
  uint32_t z_index = axl::ctz(~imm);
  uint64_t z_imm = imm ^ ((uint64_t(1) << z_index) - 1);
  uint32_t z_count = (z_imm ? axl::ctz(z_imm) : width) - z_index;

  uint32_t o_index = z_index + z_count;
  uint64_t o_imm = ~(z_imm ^ axl::trailing_bits_run<uint64_t>(o_index));
  uint32_t o_count = (o_imm ? axl::ctz(o_imm) : width) - (o_index);

  // Verify whether the bit-pattern is encodable.
  uint64_t must_be_zero = o_imm ^ ~axl::trailing_bits_run<uint64_t>(o_index + o_count);
  if (must_be_zero != 0 || (z_index > 0 && width - (o_index + o_count) != 0u)) {
    return false;
  }

  out->n = width == 64;
  out->s = (o_count + z_index - 1) | (axl::neg(width * 2u) & 0x3Fu);
  out->r = width - o_index;
  return true;
}
...
static ASMJIT_INLINE bool is_byte_mask_imm(const T& imm) noexcept {
  constexpr T kMask = T(0x0101010101010101 & axl::bit_ones<T>);
...
static ASMJIT_INLINE bool is_fp_imm8_generic(T val) noexcept {
  constexpr uint32_t kAllBsMask = axl::trailing_bits_run_const<uint32_t>(kNumBBits);
  constexpr uint32_t kB0Pattern = axl::bit_mask<uint32_t>(kNumBBits - 1);
  constexpr uint32_t kB1Pattern = kAllBsMask ^ kB0Pattern;

  T imm_z = val & axl::trailing_bits_run<T>(kNumZeroBits);
...
//! \overload
[[nodiscard]]
static ASMJIT_INLINE bool is_fp32_imm8(float val) noexcept { return is_fp32_imm8(std::bit_cast<uint32_t>(val)); }
...
//! \overload
[[nodiscard]]
static ASMJIT_INLINE bool is_fp64_imm8(double val) noexcept { return is_fp64_imm8(std::bit_cast<uint64_t>(val)); }
...
//! \overload
[[nodiscard]]
static ASMJIT_INLINE uint32_t encode_fp64_to_imm8(double val) noexcept { return encode_fp64_to_imm8(std::bit_cast<uint64_t>(val)); }
...
#endif // ASMJIT_ARM_ARM_UTILS_H_INCLUDED
