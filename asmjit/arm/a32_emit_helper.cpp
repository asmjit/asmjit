...
Error EmitHelper::emit_arg_move(
  ...
  axl::maybe_unused(dst_, dst_type_id, src_, src_type_id, comment);
  return make_error(Error::kInvalidState);
}
...
struct PrologEpilogInfo {
  ...
  axl::Array<GroupData, 2> groups;
  uint32_t sizeTotal;

  Error init(const FuncFrame& frame) noexcept {
    uint32_t offset = 0;

    for (RegGroup group : axl::EnumValues<RegGroup, RegGroup::kGp, RegGroup::kVec>{}) {
      ...
      if (group == RegGroup::kGp && frame.hasPreservedFP()) {
        ...
        savedRegs &= ~axl::bitMask(Gp::kIdFp, Gp::kIdLr);
      }

      axl::BitWordIterator<uint32_t> it(savedRegs);
      while (it.hasNext()) {
...
ASMJIT_FAVOR_SIZE Error EmitHelper::emitProlog(const FuncFrame& frame) {
  ...
  static const axl::Array<Reg, 2> groupRegs = {{ x0, d0 }};
  static const axl::Array<LoadStoreInstructions, 2> groupInsts = {{
...
static Error ASMJIT_CDECL Emitter_emit_prolog(BaseEmitter* emitter, const FuncFrame& frame) {
  ...
  axl::maybe_unused(emitter, frame);
  return make_error(Error::kInvalidState);
}
...
static Error ASMJIT_CDECL Emitter_emit_args_assignment(BaseEmitter* emitter, const FuncFrame& frame, const FuncArgsAssignment& args) {
  ...
  axl::maybe_unused(emitter, frame, args);
  return make_error(Error::kInvalidState);
}
