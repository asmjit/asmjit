...
#include <asmjit/arm/a32_inst_api_p.h>
...
Error validate(const BaseInst& inst, const Operand_* operands, size_t op_count, ValidationFlags validation_flags) noexcept {
  ...
  axl::maybe_unused(inst, operands, op_count, validation_flags);
  ...
  if (index < 0xFFu) {
    ...
    for (size_t i = 0; i < op_count; i++) {
      ...
      if (srcOp.is_mem()) {
        ...
        uint64_t accessMask = uint64_t(axl::trailing_bits_run_nz<uint32_t>(elementSize)) << (element_index * elementSize);
...
Error query_features(const BaseInst& inst, const Operand_* operands, size_t op_count, CpuFeatures* out) noexcept {
  // TODO: AArch32 tooling.
  axl::maybe_unused(inst, operands, op_count, out);
