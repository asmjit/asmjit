...
void UniCompiler::emit_3i(UniOpRRR op, const Gp& dst, const Operand_& src1_, const Operand_& src2_) {
  ...
  if (src1.is_reg() && src2.is_imm()) {
    ...
    switch (op) {
      ...
      case UniOpRRR::kMul: {
        int64_t val = b.value();
        if (dst_is_a && axl::is_power_of_2(uint64_t(val))) {
          cc->shl(dst, axl::ctz(val));
          return;
        }
      ...
      case UniOpRRR::kRol: {
        if (has_bmi2()) {
          ...
          uint32_t imm = (reg_size - b.value_as<uint32_t>()) & axl::trailing_bits_run<uint32_t>(reg_size);
          cc->rorx(dst, a, imm);
        }
        else {
          if (!dst_is_a)
    ...
    Gp b_tmp = new_similar_reg(dst, "@bImm");
    cc->mov(b_tmp, b);
    src2 = b_tmp;
  }
...
void UniCompiler::add_ext(const Gp& dst, const Gp& src_, const Gp& idx_, uint32_t scale, int32_t disp) {
  ...
  switch (scale) {
    ...
    case 4:
    case 8:
      lea(dst, x86::ptr(src, idx, axl::ctz(scale), disp));
      return;
  ...
  if (has_avx()) {
    ...
    switch (op) {
      ...
      case UniOpVV::kCvtRoundF64ToI32Lo: {
        ...
        uint32_t dst_size = axl::max(dst.size() / 2u, src.x86_rm_size());
  ...
  if (has_avx()) {
    ...
    switch (op) {
      ...
      case UniOpVVI::kSraI64: {
        ...
        if (imm <= 32) {
          cc->emit(Inst::kIdVpsrad, tmp, src, axl::min<uint32_t>(imm, 31u));
    ...
    switch (op) {
      case UniOpVVI::kSraI64: {
        ...
        if (imm <= 32 && has_sse4_1()) {
          ...
          cc->emit(Inst::kIdPsrad, tmp, axl::min<uint32_t>(imm, 31u));
