...
#include <asmjit/ujit/uni_compiler.h>
#include <asmjit/ujit/uni_compiler_utils_p.h>
#include <asmjit/ujit/uni_condition.h>
...
Operand UniCompiler::simd_const(const void* c, Bcst bcst_width, const VecArray& similar_to) {
  ...
  axl::maybe_unused(bcst_width, similar_to);

  return simd_vec_const(c, bcst_width, VecWidth::k128);
}

Vec UniCompiler::simd_vec_const(const void* c, Bcst bcst_width, VecWidth const_width) {
  axl::maybe_unused(bcst_width);
  axl::maybe_unused(const_width);
...
Mem UniCompiler::simd_mem_const(const void* c, Bcst bcst_width, VecWidth const_width) {
  axl::maybe_unused(bcst_width, const_width);
...
Vec UniCompiler::_new_vec_const(const void* c, bool is_unique_const) {
  axl::maybe_unused(is_unique_const);
...
Mem UniCompiler::tmp_stack(StackId id, uint32_t size) {
  ASMJIT_ASSERT(axl::is_power_of_2(size));
  ...
  if (src2.is_imm()) {
    ...
    switch (op) {
      ...
      case UniOpRRR::kMul: {
        uint64_t value = b.value_as<uint64_t>();
        if (value > 0u) {
          if (axl::is_power_of_2(value)) {
            uint32_t shift = axl::ctz(value);
      ...
      case UniOpRRR::kSMax: {
        ...
        if (has_cssc() && axl::is_int_n<8>(value)) {
      ...
      case UniOpRRR::kUMax: {
        uint64_t value = b.value_as<uint64_t>();
        if (has_cssc() && axl::is_uint_n<8>(value)) {
      ...
      case UniOpRRR::kSBound: {
        // TODO: CSSC
        // if (has_cssc() && axl::is_uint8(value)) {
        // }
        break;
      }
    ...
    switch (op) {
      ...
      case UniOpRRR::kSBound: {
        ...
        if (dst.id() != b.id()) {
          ...
          cc->bic(dst, dst, a, a64::asr(dst.size() * 8u - 1));
          cc->csel(dst, a, dst, CondCode::kUnsignedLT);
        }
        return;
      }
...
void UniCompiler::add_scaled(const Gp& dst, const Gp& a_, int b) {
  ...
  if (axl::is_power_of_2(b)) {
    uint32_t shift = axl::ctz(b);
...
void UniCompiler::add_ext(const Gp& dst, const Gp& src_, const Gp& idx_, uint32_t scale, int32_t disp) {
  ...
  if (axl::is_power_of_2(scale)) {
    cc->add(dst, src, idx, a64::lsl(axl::ctz(scale)));
  }
  else {
    Gp tmp = new_similar_reg(dst, "@tmp");
