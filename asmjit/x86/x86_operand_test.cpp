...
TEST_CASE(x86_operand) {
  Label L(1000); // Label with some ID.

  TEST_LOG("Checking basic properties of built-in X86 registers");
  ...
  TEST_LOG("Checking if x86::reg(...) matches built-in IDs");
  EXPECT_EQ(gpb(5), bpl);
  EXPECT_EQ(gpw(5), bp);
  EXPECT_EQ(gpd(5), ebp);
  EXPECT_EQ(gpq(5), rbp);
  ...
  TEST_LOG("Checking x86::Gp register properties");
  ...
  TEST_LOG("Checking x86::Vec register properties");
  ...
  TEST_LOG("Checking x86::Mm register properties");
  ...
  TEST_LOG("Checking x86::KReg register properties");
  ...
  TEST_LOG("Checking x86::St register properties");
  ...
  TEST_LOG("Checking if default constructed regs behave as expected");
  ...
  TEST_LOG("Checking x86::Mem operand");
  Mem m;
  EXPECT_EQ(m, Mem());
