...
#include <asmjit/x86/x86_emit_helper_p.h>
...
static ASMJIT_INLINE uint32_t x86_get_force_evex3_mask_in_last_bit(InstOptions options) noexcept {
  constexpr uint32_t kVex3Bit = axl::ctz(InstOptions::kX86_Vex3);
...
static ASMJIT_INLINE bool is_mmx_or_xmm(const Reg& reg) noexcept {
  return axl::bool_or(reg.reg_type() == RegType::kX86_Mm, reg.reg_type() == RegType::kVec128);
}
...
public:
  ...
  ASMJIT_INLINE void emit_segment_override(uint32_t segment_id) noexcept {
    ...
    uint32_t prefix = segment_prefix_table[segment_id];
  ...
  ASMJIT_INLINE void emit_imm_byte_or_dword(uint64_t imm_value, uint32_t imm_size) noexcept {
    if (!imm_size) {
      return;
    }
  ...
  ASMJIT_INLINE void emit_immediate(uint64_t imm_value, uint32_t imm_size) noexcept {
#if ASMJIT_TARGET_ARCH_BITS >= 64
...
static ASMJIT_INLINE uint32_t x86_get_movabs_inst_size_64bit(uint32_t register_size, InstOptions options, const Mem& rm_rel) noexcept {
  ...
  uint32_t rex_prefix_size = register_size == 8 || axl::test(options, InstOptions::kX86_Rex);
...
static ASMJIT_INLINE bool x86_should_use_movabs(Assembler* self, X86BufferWriter& writer, uint32_t register_size, InstOptions options, const Mem& rm_rel) noexcept {
  if (self->is_32bit()) {
    ...
    return !axl::test(options, InstOptions::kX86_ModMR | InstOptions::kX86_ModRM);
  }
  else {
    // If the addressing type is REL or MOD R/M was specified then absolute mov won't be used.
    if (rm_rel.addr_type() == Mem::AddrType::kRel || axl::test(options, InstOptions::kX86_ModMR | InstOptions::kX86_ModRM)) {
      return false;
    }
    ...
    if (rm_rel.addr_type() == Mem::AddrType::kDefault && !rm_rel.has_segment() &&
      ...
      if (axl::is_int_n<32>(int64_t(rel64))) {
        return false;
      }
    }
    else {
      if (axl::is_int_n<32>(addr_value)) {
        return false;
      }
    }
...
ASMJIT_FAVOR_SPEED Error Assembler::_emit(InstId inst_id, const Operand_& o0, const Operand_& o1, const Operand_& o2, const Operand_* op_ext) {
  ...
  uint32_t rel_size = 0;             // Relative size.
  ...
  uint32_t imm_size = 0;             // Immediate size.
  ...
  if (ASMJIT_UNLIKELY(axl::test(options, kRequiresSpecialHandling))) {
    ...
    // LOCK prefix.
    if (axl::test(options, InstOptions::kX86_Lock)) {
      if (ASMJIT_UNLIKELY(!axl::test(inst_flags, InstDB::InstFlags::kLock))) {
    ...
    if (axl::test(options, InstOptions::kX86_Rep | InstOptions::kX86_Repne)) {
      if (ASMJIT_UNLIKELY(!axl::test(inst_flags, InstDB::InstFlags::kRep))) {
      ...
      writer.emit8(axl::test(options, InstOptions::kX86_Repne) ? 0xF2 : 0xF3);
    }
  }
...
CaseX86M_GPB_MulDiv:
    ...
    case InstDB::kEncodingX86Arith:
      if (isign3 == ENC_OPS2(Reg, Reg)) {
        ...
        if (!axl::test(options, InstOptions::kX86_ModRM))
          goto EmitX86R;

        // MOD/RM: Alternative encoding selected via instruction options.
        opcode += 2u;
        axl::swap(op_reg, rb_reg);
        goto EmitX86R;
      }
      ...
      if (isign3 == ENC_OPS2(Reg, Imm)) {
        ...
        if (size == 1) {
          ...
          if (size == 2) {
            ...
            bool can_transform_to_32bit = inst_id == Inst::kIdAnd && axl::is_uint_n<32>(imm_value);

            if (!axl::is_int_n<32>(imm_value)) {
          ...
          imm_size = axl::min<uint32_t>(size, 4);
          if (axl::is_int_n<8>(imm_value) && !axl::test(options, InstOptions::kLongForm))
            imm_size = 1;
        }

        // Short form - AL, AX, EAX, RAX.
        if (rb_reg == 0 && (size == 1 || imm_size != 1) && !axl::test(options, InstOptions::kLongForm)) {
      ...
      if (isign3 == ENC_OPS2(Mem, Imm)) {
        ...
        imm_value = o1.as<Imm>().value();
        imm_size = axl::min<uint32_t>(mem_size, 4);
    ...
    case InstDB::kEncodingX86Jcc:
      if (axl::test(options, InstOptions::kTaken | InstOptions::kNotTaken) && has_encoding_option(EncodingOptions::kPredictedJumps)) {
        uint8_t prefix = axl::test(options, InstOptions::kTaken) ? uint8_t(0x3E) : uint8_t(0x2E);
        writer.emit8(prefix);
      }
    ...
    case InstDB::kEncodingX86Mov:
      ...
      if (isign3 == ENC_OPS2(Reg, Imm)) {
        ...
        if (imm_size == 1) {
          ...
          if (imm_size == 8 && !axl::test(options, InstOptions::kLongForm)) {
            if (axl::is_uint_n<32>(imm_value) && has_encoding_option(EncodingOptions::kOptimizeForSize)) {
              // Zero-extend by using a 32-bit GPD destination instead of a 64-bit GPQ.
              imm_size = 4;
            }
            else if (axl::is_int_n<32>(imm_value)) {
...
CaseX86PushPop_Gp:
    ...
    case InstDB::kEncodingX86Ret:
      ...
      if (isign3 == ENC_OPS1(Imm)) {
        imm_value = o0.as<Imm>().value();
        if (imm_value == 0 && !axl::test(options, InstOptions::kLongForm)) {
          // 'ret' without immediate, change C2 to C3.
          opcode.add(1);
          goto EmitX86Op;
        }
    ...
    case InstDB::kEncodingX86Rot:
      if (o0.is_reg()) {
        ...
        if (isign3 == ENC_OPS2(Reg, Imm)) {
          ...
          if (imm_value == 1 && !axl::test(options, InstOptions::kLongForm))
            goto EmitX86R;
    ...
    case InstDB::kEncodingX86Test:
      ...
      if (isign3 == ENC_OPS2(Reg, Imm)) {
        ...
        if (o0.x86_rm_size() == 1) {
          ...
          imm_value = o1.as<Imm>().value();
          imm_size = axl::min<uint32_t>(o0.x86_rm_size(), 4);
        }

        // Short form - AL, AX, EAX, RAX.
        if (rb_reg == 0 && !axl::test(options, InstOptions::kLongForm)) {
          opcode &= Opcode::kPP_66 | Opcode::kW;
    ...
    case InstDB::kEncodingX86Xadd:
      if (isign3 == ENC_OPS2(Reg, Reg)) {
        ...
        if (inst_id == Inst::kIdXchg && (op_reg == 0 || rb_reg == 0)) {
          ...
          else if (!axl::test(options, InstOptions::kLongForm)) {
...
CaseFpuArith_Mem:
    ...
    case InstDB::kEncodingExtMov:
      ...
      if (isign3 == ENC_OPS2(Reg, Reg)) {
        ...
        if (!axl::test(options, InstOptions::kX86_ModMR) || !inst_info->_alt_opcode_index)
          goto EmitX86R;
    ...
    case InstDB::kEncodingExtMovq:
      if (isign3 == ENC_OPS2(Reg, Reg)) {
        ...
        if (o0.is_mm_reg() && o1.is_mm_reg()) {
          ...
          if (!axl::test(options, InstOptions::kX86_ModMR))
            goto EmitX86R;
...
CaseExtRm:
    ...
    case InstDB::kEncodingExtRm_P:
      if (isign3 == ENC_OPS2(Reg, Reg)) {
        opcode.add_66h_if(axl::bool_or(o0.is_vec128(), o1.is_vec128()));
    ...
    case InstDB::kEncodingVexMr_VM:
      if (isign3 == ENC_OPS2(Mem, Reg)) {
        opcode |= axl::max(opcode_l_by_vmem(o0), opcode_l_by_size(o1.x86_rm_size()));
...
CaseVexMri:
    ...
    case InstDB::kEncodingVexRm_Wx:
      opcode.add_w_if(axl::bool_or(o0.is_gp64(), o1.is_gp64()));
      goto CaseVexRm;
...
CaseVexRm:
    ...
    case InstDB::kEncodingVexRm_VM:
      if (isign3 == ENC_OPS2(Reg, Mem)) {
        opcode |= axl::max(opcode_l_by_vmem(o1), opcode_l_by_size(o0.x86_rm_size()));
        op_reg = o0.id();
        rm_rel = &o1;
        goto EmitVexEvexM;
      }
...
CaseVexRvm_R:
    ...
    case InstDB::kEncodingVexRvm_Wx: {
      opcode.add_w_if(axl::bool_or(o0.is_gp64(), o2.x86_rm_size() == 8));
      goto CaseVexRvm;
    }
...
VexRvmi:
    ...
    case InstDB::kEncodingVexRmv_Wx:
      opcode.add_w_if(axl::bool_or(o0.is_gp64(), o2.is_gp64()));
      [[fallthrough]];
    ...
    case InstDB::kEncodingVexRmv_VM:
      if (isign3 == ENC_OPS3(Reg, Mem, Reg)) {
        opcode |= axl::max(opcode_l_by_vmem(o1), opcode_l_by_size(o0.x86_rm_size() | o2.x86_rm_size()));
...
CaseVexVmi_AfterImm:
    ...
    case InstDB::kEncodingVexVmi4_Wx:
      opcode.add_w_if(axl::bool_or(o0.is_gp64(), o1.x86_rm_size() == 8));
      imm_value = o2.as<Imm>().value();
      imm_size = 4;
    ...
    if (rm_info & kX86MemInfo_BaseGp) {
      ...
      if (rb_reg == Gp::kIdSp || force_sib) {
        ...
        // TSIB or [XSP|R12 + DISP8|DISP32].
          ...
          if (axl::is_int_n<8>(cd_offset) && rel_offset == int32_t(uint32_t(cd_offset) << cd_shift)) {
            ...
            if (inst_id == Inst::kIdLea) {
              ...
              if (rex & kX86ByteRex) {
                ...
                *mem_op_ao_mark = uint8_t(rex);
                ...
                if (rex == kX86ByteRex && !axl::test(options, InstOptions::kX86_Rex))
...
EmitModVSib:
    ...
    if (rm_info & kBaseGpIdx) {
      ...
      else if (axl::is_int_n<8>(rel_offset)) {
        writer.emit8(mod + 0x40);
...
EmitVexOp:
  {
    ...
    if (axl::test(options, InstOptions::kX86_Vex3)) {
...
EmitVexEvexR:
  {
    ...
    if (axl::test(options, kAvx512Options)) {
      ...
      if (axl::test(options, InstOptions::kX86_ER | InstOptions::kX86_SAE)) {
        ...
        if (axl::test(options, InstOptions::kX86_ER)) {
    ...
    if (common_info->prefer_evex()) {
      if ((x & kEvexBits) == 0 && !axl::test(options, InstOptions::kX86_Vex | InstOptions::kX86_Vex3)) {
        x |= kEvexForce;
      }
    }
...
EmitVexEvexM:
...

  {
    ...
    x |= uint32_t(~common_info->flags() & InstDB::InstFlags::kVex) << (31 - axl::ctz(InstDB::InstFlags::kVex));
    ...
    if (axl::test(options, kAvx512Options)) {
      ...
      if (ASMJIT_UNLIKELY(axl::test(options, InstOptions::kX86_ER | InstOptions::kX86_SAE)))
        goto InvalidEROrSAE;
    ...
    if (x & kEvexBits) {
      ...
      if (x & 0x10000000u) {
        ...
        uint32_t broadcast_ll = (axl::max<uint32_t>(axl::ctz(broadcast_vector_size), 4) - 4) << kLLShift;

        if (broadcast_ll > (2u << kLLShift))
          goto InvalidBroadcast;

        uint32_t new_ll = axl::max(current_ll, broadcast_ll);
        x = (x & ~(uint32_t(0x3) << kLLShift)) | new_ll;

        opcode &=~uint32_t(Opcode::kCDSHL_Mask);
        opcode |= axl::ctz(broadcast_unit_size) << Opcode::kCDSHL_Shift;
      }
      else {
...
EmitJmpCall:
  {
    ...
    if (rm_rel->is_label()) {
      ...
      if (label->is_bound_to(_section)) {
        ...
        // Non-bound label or label bound to a different section.
        if (opcode8 && (!opcode.v || axl::test(options, InstOptions::kShortForm))) {
          ...
          if (ASMJIT_UNLIKELY(!opcode.v || axl::test(options, InstOptions::kShortForm)))
    ...
    if (rm_rel->is_imm()) {
      ...
      if (EmitterUtils::is_absolute_location(base_address, section_offset)) {
        ...
        if (Environment::is_32bit(arch()) || axl::is_int_n<32>(int64_t(rel64))) {
...
EmitJmpCallRel:
    if (axl::is_int_n<8>(int32_t(rel32 + inst32_size - inst8_size)) && opcode8 && !axl::test(options, InstOptions::kLongForm)) {
...
Error Assembler::align(AlignMode align_mode, uint32_t alignment) {
  ...
  if (ASMJIT_UNLIKELY(!axl::is_power_of_2_up_to(alignment, Globals::kMaxAlignment)))
    return report_error(make_error(Error::kInvalidArgument));

  uint32_t i = uint32_t(axl::align_up_diff<size_t>(offset(), alignment));
  if (i > 0) {
    ...
    switch (align_mode) {
      case AlignMode::kCode: {
        if (has_encoding_option(EncodingOptions::kOptimizedAlign)) {
          ...
          do {
            uint32_t n = axl::min<uint32_t>(i, kMaxNop_size);
