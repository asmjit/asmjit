  ...
  for (;;) {
    if (TypeUtils::is_int(dst_type_id)) {
      ...
      if (TypeUtils::is_int(src_type_id) || src_.is_mem()) {
        uint32_t mov_size = axl::min(src_size, dst_size);
        if (mov_size <= 4) {
          dst_size = 4;
        }
        ...
        src_size = axl::min(src_size, mov_size);
      ...
      src_size = axl::min(src_size, dst_size);
    ...
    if (TypeUtils::is_vec(dst_type_id)) {
      ...
      if (dst_scalar_id == TypeId::kFloat32 && src_scalar_id == TypeId::kFloat64) {
        src_size = axl::min(dst_size * 2, src_size);
        dst_size = src_size / 2;
      ...
      if (dst_scalar_id == TypeId::kFloat64 && src_scalar_id == TypeId::kFloat32) {
        src_size = axl::min(dst_size, src_size * 2) / 2;
        dst_size = src_size * 2;
      ...
      if (src.is_vec() || src.is_mem()) {
        inst_id = ids().movups_or_movaps(!axl::bool_and(src.is_mem(), src_size < _emitter->environment().stack_alignment()));
...
ASMJIT_FAVOR_SIZE Error EmitHelper::emit_prolog(const FuncFrame& frame) {
  ...
  if (frame.has_preserved_fp()) {
    gp_saved &= ~axl::bit_mask<RegMask>(Gp::kIdBp);
  ...
  {
    axl::BitWordIterator<RegMask> it(gp_saved);
    while (it.has_next()) {
  ...
  // Emit 'movxxx [zsp + X], {[x|y|z]mm, k}'.
  {
    ...
    for (RegGroup group : axl::enumerate(RegGroup(1), RegGroup::kMaxVirt)) {
      axl::BitWordIterator<RegMask> it(frame.saved_regs(group));
      if (it.has_next()) {
        Reg x_reg;
