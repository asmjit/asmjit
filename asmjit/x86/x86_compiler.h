...
#ifndef ASMJIT_X86_X86_COMPILER_H_INCLUDED
#define ASMJIT_X86_X86_COMPILER_H_INCLUDED
...
public:
  ...
  ASMJIT_INLINE_NODEBUG Gp new_gp(TypeId type_id, Args&&... args) { return new_reg<Gp>(type_id, axl::forward<Args>(args)...); }
  ...
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec(TypeId type_id, Args&&... args) { return new_reg<Vec>(type_id, axl::forward<Args>(args)...); }

  //! Creates a new mask register with `type_id` type and optional name passed via `args`.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_k(TypeId type_id, Args&&... args) { return new_reg<KReg>(type_id, axl::forward<Args>(args)...); }
  ...
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Gp new_gp8(Args&&... args) { return new_reg<Gp>(TypeId::kUInt8, axl::forward<Args>(args)...); }
  ...
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Gp new_gp16(Args&&... args) { return new_reg<Gp>(TypeId::kUInt16, axl::forward<Args>(args)...); }

  //! Creates a new 32-bit general purpose register mapped to low 32 bits of a full register (on 64-bit targets).
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Gp new_gp32(Args&&... args) { return new_reg<Gp>(TypeId::kUInt32, axl::forward<Args>(args)...); }

  //! Creates a new 64-bit general purpose register.
  //!
  //! \warning The target must be 64-bit in order to create 64-bit registers.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Gp new_gp64(Args&&... args) { return new_reg<Gp>(TypeId::kUInt64, axl::forward<Args>(args)...); }

  //! Creates a new 32-bit or 64-bit general purpose register depending on the target register width.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Gp new_gpz(Args&&... args) { return new_reg<Gp>(TypeId::kUIntPtr, axl::forward<Args>(args)...); }
  ...
  //! \ref new_gpz().
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Gp new_gp_ptr(Args&&... args) { return new_reg<Gp>(TypeId::kUIntPtr, axl::forward<Args>(args)...); }

  //! Creates a new 128-bit vector register (XMM).
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec128(Args&&... args) { return new_reg<Vec>(TypeId::kInt32x4, axl::forward<Args>(args)...); }

  //! Creates a new 128-bit vector register (XMM) that will be used for scalar 32-bit floating point operation.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec128_f32x1(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x1, axl::forward<Args>(args)...); }

  //! Creates a new 128-bit vector register (XMM) that will be used for scalar 64-bit floating point operation.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec128_f64x1(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x1, axl::forward<Args>(args)...); }

  //! Creates a new 128-bit vector register (XMM) that will be used for packed 32-bit floating point operation.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec128_f32x4(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x4, axl::forward<Args>(args)...); }

  //! Creates a new 128-bit vector register (XMM) that will be used for packed 64-bit floating point operation.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec128_f64x2(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x2, axl::forward<Args>(args)...); }

  //! Creates a new 256-bit vector register (YMM).
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec256(Args&&... args) { return new_reg<Vec>(TypeId::kInt32x8, axl::forward<Args>(args)...); }

  //! Creates a new 256-bit vector register (YMM) that will be used for packed 32-bit floating point operation.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec256_f32x8(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x8, axl::forward<Args>(args)...); }

  //! Creates a new 256-bit vector register (YMM) that will be used for packed 64-bit floating point operation.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec256_f64x4(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x4, axl::forward<Args>(args)...); }

  //! Creates a new 512-bit vector register (ZMM).
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec512(Args&&... args) { return new_reg<Vec>(TypeId::kInt32x16, axl::forward<Args>(args)...); }

  //! Creates a new 512-bit vector register (ZMM) that will be used for packed 32-bit floating point operation.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec512_f32x16(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x16, axl::forward<Args>(args)...); }

  //! Creates a new 512-bit vector register (ZMM) that will be used for packed 64-bit floating point operation.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_vec512_f64x8(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x8, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec128() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_xmm(Args&&... args) { return new_reg<Vec>(TypeId::kInt32x4, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec128_f32x1() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_xmm_ss(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x1, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec128_f64x1() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_xmm_sd(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x1, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec128_f32x4() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_xmm_ps(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x4, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec128_f64x2() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_xmm_pd(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x2, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec256() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_ymm(Args&&... args) { return new_reg<Vec>(TypeId::kInt32x8, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec256_f32x8() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_ymm_ps(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x8, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec256_f64x4() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_ymm_pd(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x4, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec512() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_zmm(Args&&... args) { return new_reg<Vec>(TypeId::kInt32x16, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec512_f32x16() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_zmm_ps(Args&&... args) { return new_reg<Vec>(TypeId::kFloat32x16, axl::forward<Args>(args)...); }

  //! Alias of \ref new_vec512_f64x8() that matches x86 architecture terminology.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Vec new_zmm_pd(Args&&... args) { return new_reg<Vec>(TypeId::kFloat64x8, axl::forward<Args>(args)...); }

  //! Creates a new 64-bit MMX register.
  //!
  //! \note MMX ISA is generally deprecated by the X86 architecture.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG Mm new_mm(Args&&... args) { return new_reg<Mm>(TypeId::kMmx64, axl::forward<Args>(args)...); }

  //! Creates a new 8-bit mask (K) register.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_k8(Args&&... args) { return new_reg<KReg>(TypeId::kMask8, axl::forward<Args>(args)...); }

  //! Creates a new 16-bit mask (K) register.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_k16(Args&&... args) { return new_reg<KReg>(TypeId::kMask16, axl::forward<Args>(args)...); }

  //! Creates a new 32-bit mask (K) register.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_k32(Args&&... args) { return new_reg<KReg>(TypeId::kMask32, axl::forward<Args>(args)...); }

  //! Creates a new 64-bit mask (K) register.
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_k64(Args&&... args) { return new_reg<KReg>(TypeId::kMask64, axl::forward<Args>(args)...); }

  //! Creates a new 8-bit mask (K) register, alias of \ref new_k8().
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_kb(Args&&... args) { return new_reg<KReg>(TypeId::kMask8, axl::forward<Args>(args)...); }

  //! Creates a new 16-bit mask (K) register, alias of \ref new_k16().
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_kw(Args&&... args) { return new_reg<KReg>(TypeId::kMask16, axl::forward<Args>(args)...); }

  //! Creates a new 32-bit mask (K) register, alias of \ref new_k32().
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_kd(Args&&... args) { return new_reg<KReg>(TypeId::kMask32, axl::forward<Args>(args)...); }

  //! Creates a new 64-bit mask (K) register, alias of \ref new_k64().
  template<typename... Args>
  ASMJIT_INLINE_NODEBUG KReg new_kq(Args&&... args) { return new_reg<KReg>(TypeId::kMask64, axl::forward<Args>(args)...); }
...
#endif // ASMJIT_X86_X86_COMPILER_H_INCLUDED
