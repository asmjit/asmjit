...
#include <asmjit/x86/x86_ra_pass_p.h>
...
static ASMJIT_INLINE RATiedFlags ra_mem_base_rw_flags(OpRWFlags flags) noexcept {
  constexpr uint32_t kShift = axl::ctz(OpRWFlags::kMemBaseRW);
...
static ASMJIT_INLINE RATiedFlags ra_mem_index_rw_flags(OpRWFlags flags) noexcept {
  constexpr uint32_t kShift = axl::ctz(OpRWFlags::kMemIndexRW);
  return (RATiedFlags)ra_use_out_flags_from_rw_flags(OpRWFlags(uint32_t(flags) >> kShift) & OpRWFlags::kRW);
}
...
Error RACFGBuilder::on_instruction(InstNode* inst, InstControlFlow& cf, RAInstBuilder& ib) noexcept {
  ...
  if (Inst::is_defined_id(inst_id)) {
    ...
    ASMJIT_PROPAGATE(InstInternal::query_rw_info(_arch, inst->base_inst(), operands.data(), operands.size(), &rw_info));
      ...
      for (size_t i = 0u; i < operands.size(); i++) {
        ...
        if (op.is_reg()) {
          ...
          if (virt_index < Operand::kVirtIdCount) {
            ...
            if (rw_info.rm_feature() && axl::test(flags, RATiedFlags::kUseRM | RATiedFlags::kOutRM)) {
    ...
    if (inst->has_extra_reg()) {
      ...
      if (virt_index < Operand::kVirtIdCount) {
        ...
        RegGroup group = work_reg->group();
        RegMask in_out_regs = _pass._available_regs[group];
        uint32_t rewrite_mask = axl::bit_mask<uint32_t>(inst->_get_rewrite_index(&inst->extra_reg()._id));
    ...
    if (rw_info.has_inst_flag(InstRWFlags::kMovOp) && !inst->has_extra_reg() && axl::bit_test(op_types_mask, uint32_t(OperandType::kReg))) {
...
Error RACFGBuilder::on_invoke(InvokeNode* invoke_node, RAInstBuilder& ib) noexcept {
  ...
  for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
    ib._clobbered[group] = axl::trailing_bits_run<RegMask>(_pass._phys_reg_count.get(group)) & ~fd.preserved_regs(group);
  }
...
Error RACFGBuilder::move_vec_to_ptr(InvokeNode* invoke_node, const FuncValue& arg, const Vec& src, Out<Reg> out) noexcept {
  ...
  uint32_t arg_stack_offset = axl::align_up(invoke_node->detail()._arg_stack_size, arg_size);
...
void X86RAPass::on_init() noexcept {
  ...
  _available_regs[RegGroup::kGp] = axl::trailing_bits_run<RegMask>(_phys_reg_count.get(RegGroup::kGp));
  _available_regs[RegGroup::kVec] = axl::trailing_bits_run<RegMask>(_phys_reg_count.get(RegGroup::kVec));
  _available_regs[RegGroup::kMask] = axl::trailing_bits_run<RegMask>(_phys_reg_count.get(RegGroup::kMask)) ^ 1u;
  _available_regs[RegGroup::kX86_MM] = axl::trailing_bits_run<RegMask>(_phys_reg_count.get(RegGroup::kX86_MM));
...
Error X86RAPass::emit_swap(RAWorkReg* a_reg, uint32_t a_phys_id, RAWorkReg* b_reg, uint32_t b_phys_id) noexcept {
  bool is_64bit = axl::max(a_reg->type_id(), b_reg->type_id()) >= TypeId::kInt64;
