...
static ASMJIT_INLINE bool check_op_sig(const InstDB::OpSignature& op, const InstDB::OpSignature& ref, bool& imm_out_of_range) noexcept {
  ...
  if (!axl::test(common_flags, InstDB::OpFlags::kOpMask)) {
  ...
  if (axl::test(common_flags, InstDB::OpFlags::kMemMask)) {
  ...
  if (axl::test(common_flags, InstDB::OpFlags::kRegMask)) {
    if (ref.reg_mask() && !axl::test(op.reg_mask(), ref.reg_mask())) {
      return false;
    }
  }
...
static ASMJIT_FAVOR_SIZE Error validate(InstDB::Mode mode, const BaseInst& inst, const Operand_* operands, size_t op_count, ValidationFlags validation_flags) noexcept {
    ...
    switch (op.op_type()) {
      case OperandType::kReg: {
        ...
        if (reg_id < Operand::kVirtIdMin) {
          ...
          if (ASMJIT_UNLIKELY(axl::bit_test(vd->allowed_reg_mask[size_t(reg_type)], reg_id) == 0)) {
            return make_error(Error::kInvalidPhysId);
          }

          reg_mask = axl::bit_mask<RegMask>(reg_id);
      ...
      case OperandType::kMem: {
        ...
        if (base_type != RegType::kNone && base_type > RegType::kLabelTag) {
          ...
          else if (ASMJIT_UNLIKELY(!axl::bit_test(vd->allowed_mem_base_regs, base_type))) {
          ...
          if (base_id < Operand::kVirtIdMin) {
            ...
            // Physical base id.
            reg_mask = axl::bit_mask<RegMask>(base_id);
            combined_reg_mask |= reg_mask;
          }
          else {
        ...
        if (index_type != RegType::kNone) {
          if (ASMJIT_UNLIKELY(!axl::bit_test(vd->allowed_mem_index_regs, index_type))) {
            return make_error(Error::kInvalidAddress);
          }
          ...
          if (base_type == RegType::kPC && axl::test(op_flags, InstDB::OpFlags::kVmMask)) {
            return make_error(Error::kInvalidAddress);
          }
          ...
          if (index_id < Operand::kVirtIdMin) {
            ...
            combined_reg_mask |= axl::bit_mask<RegMask>(index_id);
          }
  ...
  if (mode == InstDB::Mode::kX86) {
    ...
    if (ASMJIT_UNLIKELY(axl::test(combined_op_flags, InstDB::OpFlags::kRegGpq))) {
    ...
    if (ASMJIT_UNLIKELY(has_rex && axl::test(combined_op_flags, InstDB::OpFlags::kRegGpbHi))) {
...
Next:
  ...
  if (axl::test(options, kAvx512Options)) {
    if (common_info.has_flag(InstDB::InstFlags::kEvex)) {
      // Validate AVX-512 {z}.
      if (axl::test(options, InstOptions::kX86_ZMask)) {
        if (ASMJIT_UNLIKELY(axl::test(options, InstOptions::kX86_ZMask) && !common_info.has_avx512_z())) {
          return make_error(Error::kInvalidKZeroUse);
        }
      }

      // Validate AVX-512 {sae} and {er}.
      if (axl::test(options, InstOptions::kX86_SAE | InstOptions::kX86_ER)) {
      ...
      if (axl::test(options, kAvx512Options) || !axl::test(options, kRepAny)) {
        return make_error(Error::kInvalidInstruction);
      }
    }
  }
  ...
  if (extra_reg.is_reg()) {
    if (axl::test(options, kRepAny)) {
      ...
      if (ASMJIT_UNLIKELY(axl::test(inst_flags, InstDB::InstFlags::kRepIgnored))) {
...
#ifndef ASMJIT_NO_INTROSPECTION
static const axl::InplaceArray<uint64_t, uint32_t(RegGroup::kMaxValue) + 1> rw_reg_group_byte_mask_table = {{
...
static ASMJIT_INLINE void rw_zero_extend_avx_vec(OpRWInfo& op_rw_info, const Vec& reg) noexcept {
  axl::maybe_unused(reg);

  uint64_t msk = ~axl::fill_trailing_bits(op_rw_info.write_byte_mask());
  if (msk) {
    op_rw_info.add_op_flags(OpRWFlags::kZExt);
...
static ASMJIT_INLINE void rw_zero_extend_non_vec(OpRWInfo& op_rw_info, const Reg& reg) noexcept {
  uint64_t msk = ~axl::fill_trailing_bits(op_rw_info.write_byte_mask()) & rw_reg_group_byte_mask_table[reg.reg_group()];
  if (msk) {
    op_rw_info.add_op_flags(OpRWFlags::kZExt);
    op_rw_info.set_extend_byte_mask(msk);
  }
...
Error query_rw_info(Arch arch, const BaseInst& inst, const Operand_* operands, size_t op_count, InstRWInfo* out) noexcept {
  ...
  if (inst_rw_info.category <= uint32_t(InstDB::RWInfo::kCategoryGenericEx)) {
    ...
    for (i = 0; i < op_count; i++) {
      ...
      op_type_mask |= axl::bit_mask<uint32_t>(src_op.op_type());
      ...
      if (op.is_read()  && !r_byte_mask) {
        r_byte_mask = axl::trailing_bits_run<uint64_t>(src_op.x86_rm_size());
      }

      if (op.is_write() && !w_byte_mask) {
        w_byte_mask = axl::trailing_bits_run<uint64_t>(src_op.x86_rm_size());
      }
      ...
      if (src_op.is_reg()) {
        ...
        if (op.is_write()) {
          ...
          else if (axl::test(rw_op_data.flags, OpRWFlags::kZExt)) {
        ...
        rm_max_size  = axl::max(rm_max_size, src_op.x86_rm_size());
        rm_ops_mask |= axl::bit_mask<uint32_t>(i);
      }
      else {
    ...
    if (out->has_inst_flag(InstRWFlags::kMovOp)) {
      if (!(op_count >= 2 && op_type_mask == axl::bit_mask<uint32_t>(OperandType::kReg) && has_same_reg_type(reinterpret_cast<const Reg*>(operands), op_count))) {
    ...
    if (rm_ops_mask && !inst.has_option(InstOptions::kX86_ER)) {
      axl::BitWordIterator<uint32_t> it(rm_ops_mask);
      do {
        i = it.next();
    ...
    case InstDB::RWInfo::kCategoryImul: {
      ...
      if (op_count == 2) {
        ...
        if (operands[0].is_gp16() && operands[1].x86_rm_size() == 1) {
          ...
          out->_operands[0].set_read_byte_mask(axl::trailing_bits_run<uint64_t>(1));
    ...
    case InstDB::RWInfo::kCategoryMovh64: {
      ...
      if (op_count == 2) {
        if (operands[0].is_vec() && operands[1].is_mem()) {
          ...
          out->_operands[0].set_write_byte_mask(axl::trailing_bits_run<uint64_t>(8) << 8);
        ...
        if (operands[0].is_mem() && operands[1].is_vec()) {
          ...
          out->_operands[1].set_read_byte_mask(axl::trailing_bits_run<uint64_t>(8) << 8);
          return Error::kOk;
        }
      }
      break;
    ...
    case InstDB::RWInfo::kCategoryVmovmskps: {
      ...
      if (op_count == 2) {
        if (operands[0].is_gp() && operands[1].is_vec()) {
          ...
          out->_operands[0].set_extend_byte_mask(axl::trailing_bits_run<uint32_t>(native_gp_size - 1) << 1);
          out->_operands[1].reset(R, operands[1].x86_rm_size());
          return Error::kOk;
        }
      }
...
struct RegAnalysis {
  ...
  inline bool has_reg_type(RegType reg_type) const noexcept {
    return axl::bit_test(reg_type_mask, reg_type);
  }
};
...
static RegAnalysis InstInternal_reg_analysis(const Operand_* operands, size_t op_count) noexcept {
  ...
  for (uint32_t i = 0; i < op_count; i++) {
    ...
    if (op.is_reg()) {
      ...
      mask |= axl::bit_mask<uint32_t>(reg.reg_type());
      if (reg.is_vec()) {
      ...
      if (mem.has_base_reg()) {
        mask |= axl::bit_mask<uint32_t>(mem.base_type());
      }
      if (mem.has_index_reg()) {
        mask |= axl::bit_mask<uint32_t>(mem.index_type());
...
static inline uint32_t InstInternal_usesAvx512(InstOptions inst_options, const RegOnly& extra_reg, const RegAnalysis& reg_analysis) noexcept {
  ...
  uint32_t has_k_or_zmm = reg_analysis.reg_type_mask & axl::bit_mask<uint32_t>(RegType::kVec512, RegType::kMask);
...
Error query_features(Arch arch, const BaseInst& inst, const Operand_* operands, size_t op_count, CpuFeatures* out) noexcept {
  ...
  if (feature_data != additional_info.features_begin()) {
    ...
    if (out->has(Ext::kVPCLMULQDQ)) {
      if (reg_analysis.has_reg_type(RegType::kVec512) || axl::test(options, InstOptions::kX86_Evex)) {
        // AVX512_F & VPCLMULQDQ.
    ...
    if (out->has(Ext::kAVX) && out->has(Ext::kAVX2)) {
      ...
      if (inst_id == Inst::kIdVbroadcastss || inst_id == Inst::kIdVbroadcastsd) {
        ...
        // instruction.
        if (!(reg_analysis.reg_type_mask & axl::bit_mask<uint32_t>(RegType::kVec256, RegType::kVec512))) {
          is_avx2 = false;
        }
      }
    ...
    if (out->has_any(Ext::kAVX,
      ...
      if (inst_info.common_info().prefer_evex() && !axl::test(options, InstOptions::kX86_Vex | InstOptions::kX86_Vex3))
        use_evex = 1;
