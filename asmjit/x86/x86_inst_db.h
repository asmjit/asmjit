...
#ifndef ASMJIT_X86_X86_INST_DB_H_INCLUDED
#define ASMJIT_X86_X86_INST_DB_H_INCLUDED
...
enum class InstFlags : uint32_t {
  ...
  //! FPU instruction can address `word_ptr` (shared with M80 for compactness).
  kFpuM16 = 0x00000800u,
  //! FPU instruction can address `dword_ptr`.
  kFpuM32 = 0x00001000u,
  //! FPU instruction can address `qword_ptr`.
  kFpuM64 = 0x00002000u,
  //! FPU instruction can address `tword_ptr` (shared with M16 for compactness).
  kFpuM80 = 0x00000800u,
...
struct CommonInfo {
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_flag(InstFlags flag) const noexcept { return axl::test(_flags, flag); }
  ...
  [[nodiscard]]
  ASMJIT_INLINE_NODEBUG bool has_avx512_flag(Avx512Flags flag) const noexcept { return axl::test(_avx512_flags, flag); }
  ...
  ASMJIT_INLINE_NODEBUG uint32_t broadcast_size() const noexcept {
    constexpr uint32_t kShift = axl::ctz(Avx512Flags::kB16);
...
#endif // ASMJIT_X86_X86_INST_DB_H_INCLUDED
