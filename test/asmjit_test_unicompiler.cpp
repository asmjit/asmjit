...
static void printAppInfo() noexcept {
  printf("AsmJit UniCompiler Test Suite v%u.%u.%u [Arch=%s] [Mode=%s]\n\n",
    ...
    asmjitArchAsString(asmjit::Arch::kHost),
    asmjitBuildType()
  );
}
...
static ASMJIT_INLINE_CONSTEXPR uint32_t byteWidthFromVecWidth(VecWidth vw) noexcept {
...
#if !defined(ASMJIT_NO_LOGGING)
  const char* loggerData() const noexcept { return logger.data(); }
#else
  const char* loggerData() const noexcept { return "<ASMJIT_NO_LOGGING>"; }
#endif
};
...
static TestCondRRFunc create_func_cond_rr(JitContext& ctx, UniOpCond op, CondCode condCode, uint32_t variation) noexcept {
  ctx.prepare();
  UniCompiler pc(&ctx.cc, ctx.features, ctx.optFlags);
...
static TestCondRIFunc create_func_cond_ri(JitContext& ctx, UniOpCond op, CondCode condCode, Imm bImm) noexcept {
  ctx.prepare();
  UniCompiler pc(&ctx.cc, ctx.features, ctx.optFlags);
...
static ASMJIT_NOINLINE void test_conditional_op(JitContext& ctx, UniOpCond op, CondCode condCode, int32_t a, int32_t b, bool expected) noexcept {
...
struct alignas(16) VecOverlay {
  ...
  ASMJIT_INLINE_NODEBUG void copy16bFrom(const VecOverlay<kOtherW>& other) noexcept {
...
template<uint32_t kW>
static ASMJIT_NOINLINE String vec_stringify(const VecOverlay<kW>& vec, VecElementType elementType) noexcept {
  String s;
  s.append('{');
...
public:
  ...
  ASMJIT_NOINLINE uint64_t nextUInt64() noexcept {
    if (++step >= 256) {
      step = 0;
    }
  ...
  ASMJIT_NOINLINE float nextFloat32() noexcept {
    if (++step >= 256) {
      step = 0;
    }
  ...
  ASMJIT_NOINLINE double nextFloat64() noexcept {
    if (++step >= 256) {
      step = 0;
    }
...
struct ConstraintNone {
  template<uint32_t kW>
  static ASMJIT_INLINE_NODEBUG void apply(VecOverlay<kW>& v) noexcept { DebugUtils::unused(v); }
};
...
template<typename T> struct vec_op_adds : public op_each_vvv<T, vec_op_adds<T>> {
  static ASMJIT_INLINE T apply_one(const T& a, const T& b) noexcept {
    ...
    T result = Support::addOverflow(a, b, &of);
...
template<typename T> struct vec_op_subs : public op_each_vvv<T, vec_op_subs<T>> {
  static ASMJIT_INLINE T apply_one(const T& a, const T& b) noexcept {
    Support::FastUInt8 of{};
    T result = Support::subOverflow(a, b, &of);
...
template<typename T> struct vec_op_cmp_eq : public op_each_vvv<T, vec_op_cmp_eq<T>> {
  static ASMJIT_INLINE_NODEBUG T apply_one(const T& a, const T& b) noexcept { return a == b ? Support::allOnes<T>() : T(0); }
};
...
static ASMJIT_NOINLINE void test_vecop_vv_failed(UniOpVV op, Variation variation, const VecOverlay<kW>& arg0, const VecOverlay<kW>& observed, const VecOverlay<kW>& expected, const char* assembly) noexcept {
  ...
  String arg0_str = vec_stringify(arg0, opInfo.arg(0));
  String observed_str = vec_stringify(observed, opInfo.ret());
  String expected_str = vec_stringify(expected, opInfo.ret());
...
static ASMJIT_NOINLINE void test_vecop_vvv_failed(UniOpVVV op, Variation variation, const VecOverlay<kW>& arg0, const VecOverlay<kW>& arg1, const VecOverlay<kW>& observed, const VecOverlay<kW>& expected, const char* assembly) noexcept {
  ...
  String arg0_str = vec_stringify(arg0, opInfo.arg(0));
  String arg1_str = vec_stringify(arg1, opInfo.arg(1));
  String observed_str = vec_stringify(observed, opInfo.ret());
  String expected_str = vec_stringify(expected, opInfo.ret());
...
static ASMJIT_NOINLINE void test_vecop_vvvv_failed(UniOpVVVV op, Variation variation, const VecOverlay<kW>& arg0, const VecOverlay<kW>& arg1, const VecOverlay<kW>& arg2, const VecOverlay<kW>& observed, const VecOverlay<kW>& expected, const char* assembly) noexcept {
  ...
  String arg0_str = vec_stringify(arg0, opInfo.arg(0));
  String arg1_str = vec_stringify(arg1, opInfo.arg(1));
  String arg2_str = vec_stringify(arg2, opInfo.arg(2));
  String observed_str = vec_stringify(observed, opInfo.ret());
  String expected_str = vec_stringify(expected, opInfo.ret());
...
static ASMJIT_NOINLINE void test_vecop_vv_constraint(JitContext& ctx, Variation variation = Variation{0}) noexcept {
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected, opInfo.ret())) {
      test_vecop_vv_failed(kOp, variation, a, observed, expected, ctx.loggerData());
    }
  }
...
static ASMJIT_NOINLINE void test_vecop_vvi_constraint(JitContext& ctx, uint32_t imm, Variation variation = Variation{0}) noexcept {
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected, opInfo.ret())) {
      test_vecop_vvi_failed(kOp, variation, a, observed, expected, imm, ctx.loggerData());
    }
  }
...
static ASMJIT_NOINLINE void test_vecop_vvv_constraint(JitContext& ctx, Variation variation = Variation{0}) noexcept {
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected, opInfo.ret())) {
      test_vecop_vvv_failed(kOp, variation, a, b, observed, expected, ctx.loggerData());
    }
  }
...
static ASMJIT_NOINLINE void test_vecop_vvvi_constraint(JitContext& ctx, uint32_t imm, Variation variation = Variation{0}) noexcept {
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected, opInfo.ret())) {
      test_vecop_vvvi_failed(kOp, variation, a, b, observed, expected, imm, ctx.loggerData());
    }
  }
...
static ASMJIT_NOINLINE void test_vecop_vvvv_constraint(JitContext& ctx, Variation variation = Variation{0}) noexcept {
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected, opInfo.ret())) {
      test_vecop_vvvv_failed(kOp, variation, a, b, c, observed, expected, ctx.loggerData());
    }
  }
}
...
#else
  asmjit::DebugUtils::unused(features);
...
#if defined(ASMJIT_UJIT_X86)
  UniCompilerTests::test_x86_ops(ctx, hostFeatures);
#elif defined(ASMJIT_UJIT_AARCH64)
  UniCompilerTests::test_a64_ops(ctx, hostFeatures);
#endif
}
...
int main() {
  printAppInfo();
  printf("!! UniCompiler test suite is disabled: <ASMJIT_NO_[U]JIT> or unsuitable arvhitecture !!\n");
  return 0;
}
